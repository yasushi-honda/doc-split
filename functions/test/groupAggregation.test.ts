...
/**
 * buildContribution() / diffContribution() のテスト（Issue #664修正、ADR-0021）
 *
 * 背景: ADR-0020の「event.before/afterの履歴差分適用」モデルは、異なるevent.id間の
 * 配信順序不同に対して脆弱だった(`/codex plan`セカンドオピニオンで指摘)。本モデルは
 * 「前回このトリガーが適用したcontribution → 現在のdocumentsのライブ状態が示す
 * contribution」の差分を取ることで、イベント到着順序に依存せず正しく収束する。
 *
 * buildContributionはgetAffectedGroups()と異なり、保存済みのcustomerKey等ではなく
 * 生フィールド(customerName等)から毎回キーを再導出する(rebuildAllGroupAggregations()
 * 等と同じパターン)。
 */
describe('buildContribution (ライブdocumentからのcontribution算出 / Issue #664)', () => {
  it('通常の完了済み文書 → customer/office/documentType/careManagerの4件を返す', () => {
    const data = baseDoc();

    const result = buildContribution(data);

    expect(result).to.have.lengthOf(4);
    expect(result).to.deep.include({ groupType: 'customer', groupKey: '山田太郎', displayName: '山田太郎' });
    expect(result).to.deep.include({ groupType: 'careManager', groupKey: '佐藤花子', displayName: '佐藤花子' });
  });

  it('status: split → 空配列（分割元は集計対象外）', () => {
    const data = baseDoc({ status: 'split' });
  ...
  it('undefined(文書が存在しない・削除済み) → 空配列', () => {
    const result = buildContribution(undefined);

    expect(result).to.deep.equal([]);
  });

  it('保存済みのcustomerKey等が古い(生フィールドと不一致)場合でも、生フィールドから正しく再導出する', () => {
    // needsKeyUpdateがまだ反映されていないライブ読込みを模したケース
    const data = baseDoc({ customerName: '鈴木一郎', customerKey: '古い山田太郎' });

    const result = buildContribution(data);

    const customerEntry = result.find((r) => r.groupType === 'customer');
    expect(customerEntry).to.deep.equal({ groupType: 'customer', groupKey: '鈴木一郎', displayName: '鈴木一郎' });
  });

  it('careManager未設定 + customerKey非空 → CM未設定の予約keyにフォールバックする', () => {
    const data = baseDoc({ careManager: undefined, careManagerKey: '' });
  ...
  it('customerKeyも空(pending等の未処理状態) → careManagerもフォールバックせず全体が空配列', () => {
    const data = baseDoc({
      customerName: undefined, customerKey: '',
      officeName: undefined, officeKey: '',
      documentType: undefined, documentTypeKey: '',
      careManager: undefined, careManagerKey: '',
      status: 'pending',
    });
...
describe('diffContribution (contribution差分計算 / Issue #664)', () => {
  it('previous=[] target=[A] → Aにdelta+1', () => {
    const result = diffContribution(
      [],
      [{ groupType: 'customer', groupKey: 'a', displayName: 'A' }]
    );

    expect(result).to.deep.equal([{ groupType: 'customer', groupKey: 'a', displayName: 'A', delta: 1 }]);
  });

  it('previous=[A] target=[] → Aにdelta-1（削除・deleteイベント相当）', () => {
    const result = diffContribution(
      [{ groupType: 'customer', groupKey: 'a', displayName: 'A' }],
      []
    );
  ...
  it('previous/targetが完全一致(キー・表示名とも同一) → 空配列（真のno-op、Firestore書込み不要）', () => {
    const entry = { groupType: 'customer' as const, groupKey: 'a', displayName: 'A' };

    const result = diffContribution([entry], [entry]);

    expect(result).to.deep.equal([]);
  });

  it('キーは同一だが表示名のみ変化 → delta:0で新しい表示名のみ返す', () => {
    const result = diffContribution(
      [{ groupType: 'customer', groupKey: 'a', displayName: '山田太郎' }],
      [{ groupType: 'customer', groupKey: 'a', displayName: 'Ｙａｍａｄａ' }]
    );

    expect(result).to.deep.equal([
      { groupType: 'customer', groupKey: 'a', displayName: 'Ｙａｍａｄａ', delta: 0 },
    ]);
  });

  it('反例ケース(Codexセカンドオピニオン指摘): 同一groupType内でキーがA→Bへ変化 → Aに-1、Bに+1のみ（無関係な他文書分は減算されない）', () => {
    // 別文書Yが実在してグループAに1件存在する状況を模擬。文書XのA→B変化イベントを
    // 単独で処理しても、diffContribution自体はXの寄与のみを扱うため、Aへの-1は
    // 「Xがaへの寄与を持っていた場合のみ」発生し、Y分の寄与には触れない
    // (Y分はY自身のcontributionとして別途documentAggregationStates/{Yのdoc}で管理される)
    const result = diffContribution(
      [{ groupType: 'customer', groupKey: 'a', displayName: 'A' }],
      [{ groupType: 'customer', groupKey: 'b', displayName: 'B' }]
    );

    expect(result).to.have.lengthOf(2);
    expect(result).to.deep.include({ groupType: 'customer', groupKey: 'a', displayName: 'A', delta: -1 });
    expect(result).to.deep.include({ groupType: 'customer', groupKey: 'b', displayName: 'B', delta: 1 });
  });

  it('複数groupType混在: 一部は不変・一部は変化 → 変化分のみdeltaを返す', () => {
    const previous = [
      { groupType: 'customer' as const, groupKey: 'yamada', displayName: '山田太郎' },
      { groupType: 'careManager' as const, groupKey: 'sato', displayName: '佐藤花子' },
    ];
    const target = [
      { groupType: 'customer' as const, groupKey: 'yamada', displayName: '山田太郎' }, // 不変
      { groupType: 'careManager' as const, groupKey: 'suzuki', displayName: '鈴木次郎' }, // 変化
    ];

    const result = diffContribution(previous, target);

    expect(result).to.have.lengthOf(2);
    expect(result).to.deep.include({ groupType: 'careManager', groupKey: 'sato', displayName: '佐藤花子', delta: -1 });
    expect(result).to.deep.include({ groupType: 'careManager', groupKey: 'suzuki', displayName: '鈴木次郎', delta: 1 });
  });

  it('previous=[] target=[] → 空配列', () => {
    const result = diffContribution([], []);
