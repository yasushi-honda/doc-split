...
describe('processDocument() / applyOcrCompletionTransaction() faxDuplication配線契約 (GOAL.md D1-D5)', () => {
  ...
  it('offload済みOCR結果(ocrResultUrl)は各コピー専用のStorageパスへ複製し、元docのURLをそのまま共有しない(Codexセカンドオピニオン指摘P1: 元docの再処理でIssue #625 cleanupがコピー分も削除してしまう問題の修正)', () => {
    expect(source).to.match(
    ...
    expect(
      loopBlock,
      'CodeRabbit指摘反映: 複製先キーはnewDocRef.id(リトライ非決定)ではなく`${docId}-${assignment.customerId}`(リトライ安定)であること'
    ).to.match(/`\$\{docId\}-\$\{assignment\.customerId\}`,\s*\n\s*ownershipExpectation\.ocrRunId/);
    expect(
      loopBlock,
