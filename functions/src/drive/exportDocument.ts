 ...
 * 人間による明示的選択が確認できない、のいずれか)のままエクスポートが試みられた場合にthrow
 * (同姓同名リスク対応、2026-07-25)。Driveフォルダ名は`doc.customerName`文字列のみで決まり
 * customerIdを参照しないため、顧客未確定のままエクスポートすると、OCRが任意に選んだ顧客情報の
 * まま外部Driveへ書類が渡ってしまう。
 *
 * OCRが別名(例:「田中一郎」/「田中一朗」)をスコア僅差で取り違えるケースは対象外(既存のOCR精度
 * 課題として別途扱う、ADR-0022参照)。判定の詳細は`customerAmbiguityGate.ts`を参照
 * (2026-07-25追記: 当初「守備範囲は同姓同名マスターの衝突のみ」と要約していたが、実データ監査で
 * 要約が実態と乖離していたと判明し訂正)。
 *
 * 呼び出し元（トリガー）はこれを捕捉し `driveExportStatus: 'error'` に遷移させる。
