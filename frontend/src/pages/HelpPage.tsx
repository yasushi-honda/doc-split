  ...
  return (
    <div className="guide-content">
      ...
      <section id="user-section-3" className="guide-section">
        ...
        <table className="guide-table">
          ...
          <tbody>
            ...
            <tr><td>書類種別</td><td>「カテゴリ → 書類種別 → 書類リスト」の3階層で表示。カテゴリが未設定の書類は末尾の「未分類」にまとまります</td></tr>
        ...
        <ul>
          ...
          <li><strong>書類種別フィルター</strong>: マスター登録された書類種別で絞り込み（書類一覧タブのみ）</li>
          <li><strong>ケアマネジャーフィルター</strong>: 担当ケアマネで絞り込み（書類一覧タブのみ、他フィルターと併用可）</li>
          <li><strong>未確認のみ表示</strong>: 確認済みチェックが未設定の書類のみ表示</li>
        </ul>
      ...
      <section id="user-section-4" className="guide-section">
        ...
        <h3>4.4 メタ情報の編集</h3>
        <p>OCRの認識結果を後から修正したい場合、書類詳細モーダル右上の「編集」ボタンで編集モードに切り替えられます。</p>
        ...
        <ul>
          ...
          <li>書類種別（マスターから選択）</li>
          <li>事業所（マスターから選択）</li>
          <li>担当ケアマネ（マスターから選択）</li>
          <li>書類日付</li>
        </ul>

        <div className="info-box">
          <h4>編集はこの書類のみに反映されます</h4>
          <p>
            ここでの編集は<strong>この書類個別の記録のみ</strong>更新します。
            顧客・事業所・書類種別・担当ケアマネのマスタは変更されません。
            マスタ自体を編集したい場合は、マスター管理画面（管理者権限が必要）から行ってください。
          </p>
        </div>

        <p className="note">※顧客を変更した際、担当ケアマネが空欄であればその顧客の担当ケアマネが自動で補完されます（既に値が入っている場合は上書きされません）。</p>
        <p className="note">※同姓同名（完全一致）の顧客マスターが複数存在する場合、顧客欄を選び直さずに他の項目だけ保存しても「顧客確定」扱いにはなりません。該当の顧客を明示的に選択し直して保存してください。</p>
      </section>
    ...
    <div className="guide-content">
      ...
      <section id="admin-section-2b" className="guide-section">
        ...
        <div className="info-box">
          <h4>アクセス範囲について</h4>
          <p>
            DocSplitは連携したGoogleアカウントがアクセスできるDrive全体を読み書きできる権限（Googleの「drive」スコープ）で接続します。
            これは、フォルダを手動で作成・移動しても書類の振り分け先として正しく認識できるようにするためです。
            連携は書類のエクスポート専用の、業務データのみを扱うGoogleアカウントで行うことを推奨します。
          </p>
        </div>
        ...
        <p>
          ...
          「5階層（詳細）」「3階層（シンプル）」のひな形カードをクリックすると、それぞれの標準構成をワンクリックで反映できます。
          反映直後は1階層目のフォルダ名が未入力のため、必ず入力してください
          （それ以外の階層構成は必要に応じて個別にカスタマイズも可能です）。
        </p>
        <ol className="steps-list">
          <li>ひな形カードをクリックしてテンプレートを初期化する（または個別に階層を組み立てる）</li>
        ...
        <div className="info-box">
          ...
          <ul>
            ...
            <li>顧客が確定できない場合もエラーとなります（顧客名が未設定または「未判定」「不明顧客」のまま／顧客マスターが改名され書類側の顧客名と食い違っている／同姓同名（完全一致）のマスターが複数あり選び直されていない、のいずれか）。書類詳細画面で該当の顧客を選び直して保存すると確定した扱いになり、自動で再エクスポートされます</li>
          </ul>
        </div>
