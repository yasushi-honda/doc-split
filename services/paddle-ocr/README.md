...
いずれのtierもtrial単位の完了率も100%(1p:30/30、71p:20/20、20p:20/20、160p:2/2)であり、page単位・trial単位の両方でリクエスト失敗は一件も発生していない。

### この測定が示すこと・示さないこと

- **示すこと**: 本移行(Pass1切替、PR6着手)の必須ゲートである71ページが、N=20試行・page単位1,420リクエストで完了率100%・p95=600.5秒(基準850秒に対し約29%のマージン)を達成した。20ページも同様に十分なマージンでPASSした。N=20は統計的証明ではなくPR6着手前の実用性スクリーニングである点に留意(20/20成功でも完了率95%の両側95%信頼区間下限は約83%)。
...
- **cold start(バースト1回目)が全4tierで一貫して外れ値化する現象を確認**: 1p=33.4秒、71p=51.3秒、20p=38.0秒、160p=28.0秒。いずれもバースト1回目のみ突出し、2回目以降は9〜10秒台に収束した。cold候補測定はページ数によらず単一ページ相当の応答時間を計測する設計のため、ページ数と相関しないこの傾向は、Cloud Runインスタンス起動そのもの(コンテナpull・モデルロード)に起因する変動と考えられる。1ページのcoldゲート(≤30秒)はこの変動によりFAILした。1ページは実運用の最頻ケース(57%)であり、Issue #966として起票。**追加スパイク検証(2026-09-19、dev環境で`min-instances=1`に変更して実施)**: 3秒間隔の孤立した単発リクエスト(本番の実運用パターンに近い)を3回送信したところ全て6.2〜6.4秒で安定し外れ値は解消した。一方、意図的な3件同時バースト(`--concurrency=1`の上限を超える負荷)では34.5秒の外れ値が引き続き発生した(常時起動インスタンスを超える分の新規起動はmin-instancesでは解消しないため、機序として妥当)。本番で単一ページ文書が真に同時到達するケースは稀なため、decision-maker判断により`min-instances=1`を恒久採用(`deploy-paddle-ocr.yml`)。常時1インスタンス分のCloud Run課金が発生する(下記「コスト監視」節参照)。
- p95は右側打ち切り(未完走trialを+Infinity扱いで順位統計に含める)で算出する設計だが、今回は全trial完走のため打ち切りの影響はない。

### コスト実測

事前見積もり(`--cpu=4`反映後、全tier合計約2,190リクエスト×約6.5秒×4vCPU≒約56,800 vCPU秒、月間無料枠180,000の約32%、実費換算$2未満)に対し、実測のリクエスト総数(warm: 30+20+20+2=72trial、cold: (5バースト×3リクエスト)×4tier=60リクエスト、うち71ページwarmだけで20試行×71ページ=1,420ページ相当)は見積もり時の想定巡回範囲内に収まった。GCP Billingへの反映には日次ラグがあり詳細な請求額は未確認だが、桁レベルの判断(無料枠内、実費数ドル未満)を覆す規模のずれはない。
...
- **`min-instances=1`(2026-09-19採用)**: 1ページcold start変動(Issue #966)への緩和策として、リクエスト有無によらず常時1インスタンスを起動状態に保つ設定へ変更した(以前は`min-instances=0`、トラフィックなしでスケールゼロ)。CPU=4/メモリ=4Giのインスタンスが24時間365日起動する分の追加課金が発生する(具体額はGCP Billing反映後に確認)。dev/kanameone/cocoro全環境の`deploy-paddle-ocr.yml`が共通のためkanameone/cocoroへも同様に適用される(現時点ではPass1未切替のためトラフィックは流れていないが、インスタンス起動コストは発生する)
