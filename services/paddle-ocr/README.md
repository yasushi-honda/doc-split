...
- **cold start(バースト1回目)が全4tierで一貫して外れ値化する現象を確認**: 1p=33.4秒、71p=51.3秒、20p=38.0秒、160p=28.0秒。いずれもバースト1回目のみ突出し、2回目以降は9〜10秒台だった。**この「2回目以降」の解釈には注意が必要**: バースト間隔(`COLD_BURST_COOLDOWN_MS=180秒`)がCloud Runのインスタンス保持時間より短い可能性があり、2〜5回目は真のcold startではなく1回目で起動したインスタンスがまだ温かいまま応答した値である疑いが残る(warm p95=9.26秒とほぼ同値)。その場合、各tierの真のcold標本は実質N=1(1回目の値)であり、この間隔設定の妥当性は未検証のまま残る。1ページのcoldゲート(≤30秒)はこの変動によりFAILした。1ページは実運用の最頻ケース(57%)であり、Issue #966として起票。
  ...
  - **本番での実際のリスク再評価(訂正、2026-09-19)**: 当初「Gmail添付ポーリング(`checkGmailAttachments.ts`、`maxResults=50`)が`processOCROnCreate.ts`の`onDocumentCreated`トリガー経由で並行起動する」ことをバーストリスクの根拠として記載していたが、この根拠は誤りだった。`processOCROnCreate`は`functions/src/index.ts`でexportされておらず(ADR-0010、2026-02-08で廃止済み)、実際にはデプロイされていない(dev環境で`gcloud functions list`により未存在を確認)。
    - **正しいメカニズム**: 本番の唯一のOCRエントリーポイントは`processOCR`(Cloud Scheduler、1分間隔ポーリング)。1サイクルは`for`ループで完全に逐次処理される(最大5件、1件ずつ、文書間2秒delay)。ただし実機設定は`maxInstanceCount=1`かつ`maxInstanceRequestConcurrency=80`(既定値)であり、**`maxInstances=1`はインスタンス数の上限であって同時リクエスト数の制限ではない**。1サイクルの所要時間がポーリング間隔(60秒)を超えると、次のtickが同一インスタンス内で並行実行され、別文書に対するOCRリクエストが同時in-flightになりうる。ADR-0023(2026-08-02)はこの現象がkanameone本番で実際に観測されたことを記録しており、`concurrency=1`の明示設定を意図的にスコープ外として先送りしている
    - **現状(Gemini運用下)**: kanameone実測(2026-09-18)で1サイクル19〜33秒程度と60秒に余裕があり、tick重複は現状顕在化していない
    - **PR6後のリスク**: PaddleOCRは1件あたりの所要時間がGeminiより長く、cold start外れ値(実測27.9〜51.3秒)もあるため、Pass1切替後はこの60秒境界を超える確率が上がり、tick重複による複数文書の同時OCRリクエストが現実的なリスクになる。`concurrency=1`明示設定の要否・サイクル所要時間の監視指標化は、PR6のロールアウト計画で扱う(Issue #966参照)
    - **min-instances=1の効能(上方修正)**: 現状の実トラフィックは完全に直列(1サイクル内で次のリクエストは前の完了を待つ)であるため、min-instances=1は「バーストの1件目のみ」ではなく、**現状のトラフィック形状においては実質的に全リクエストのcold startを解消する**。decision-maker判断により恒久採用した(`deploy-paddle-ocr.yml`)。常時1インスタンス分のCloud Run課金が発生する(下記「コスト監視」節参照)。
