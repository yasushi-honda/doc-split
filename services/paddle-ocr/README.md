...
- **本番のOCRタイムアウト予算は900秒**(`processOCR`、Cloud Scheduler経路。旧540秒からADR-0023[2026-08-01の実インシデントを受けた決定]で引き上げ済み)。850秒ゲートはこの予算を対象にしている(訂正、2026-09-19: 旧記述は「onCreate初回試行→scheduledへフォールバック」という誤った2経路設計を前提にしていたが、`processOCROnCreate`はADR-0010で廃止済み・未デプロイのため、そのような経路は存在しない。本番の唯一の経路は`processOCR`自身であり、そのタイムアウトが540秒→900秒に引き上げられただけである)。
- **cold start(バースト1回目)が全4tierで一貫して外れ値化する現象を確認**: 1p=33.4秒、71p=51.3秒、20p=38.0秒、160p=28.0秒。いずれもバースト1回目のみ突出し、2回目以降は9〜10秒台だった。**この「2回目以降」の解釈には注意が必要**: バースト間隔(`COLD_BURST_COOLDOWN_MS=180秒`)がCloud Runのインスタンス保持時間より短い可能性があり、2〜5回目は真のcold startではなく1回目で起動したインスタンスがまだ温かいまま応答した値である疑いが残る(warm p95=9.26秒とほぼ同値)。その場合、各tierの真のcold標本は実質N=1(1回目の値)であり、この間隔設定の妥当性は未検証のまま残る。1ページのcoldゲート(≤30秒)はこの変動によりFAILした。1ページは実運用の最頻ケース(57%)であり、Issue #966として起票。
  ...
  - **本番での実際のリスク再評価(訂正、2026-09-19)**: 当初「Gmail添付ポーリング(`checkGmailAttachments.ts`、`maxResults=50`)が`processOCROnCreate.ts`の`onDocumentCreated`トリガー経由で並行起動する」ことをバーストリスクの根拠として記載していたが、この根拠は誤りだった。`processOCROnCreate`は`functions/src/index.ts`でexportされておらず(ADR-0010、2026-02-08で廃止済み)、実際にはデプロイされていない(dev環境で`gcloud functions list`により未存在を確認)。
    - **正しいメカニズム**: 本番の唯一のOCRエントリーポイントは`processOCR`(Cloud Scheduler、1分間隔ポーリング)。1サイクルは`for`ループで完全に逐次処理される(最大5件、1件ずつ、文書間2秒delay)。ただし実機設定は`maxInstanceCount=1`かつ`maxInstanceRequestConcurrency=80`(既定値)であり、**`maxInstances=1`はインスタンス数の上限であって同時リクエスト数の制限ではない**。1サイクルの所要時間がポーリング間隔(60秒)を超えると、次のtickが同一インスタンス内で並行実行され、別文書に対するOCRリクエストが同時in-flightになりうる。ADR-0023「concurrency設定の実態」節は、この現象がkanameone本番のログで「厳密には成立していない可能性がある」「強く示唆される」と(断定ではなくhedgeした表現で)記録しており、`concurrency=1`の明示設定を意図的にスコープ外として先送りしている
    - **再訂正(2026-09-19、2度目): 「PR6後にリスクが上がる」ではなく「現在も起こりうるが実害が顕在化していない」が正確**。ADR-0023の実測(71ページ文書のページループが319秒=4.5秒/ページ、Gemini)から、10ページ台以上の文書が1サイクルの5件に含まれれば、**Gemini運用の現在でも1サイクルが60秒を超えうる**(実際、上記ADR-0023のインシデント当日に「1サイクルの完了ログが欠落したまま次サイクルが動いている」挙動が観測されている)。文書の43%が複数ページのため、tick重複自体は既知のバースト源としては起きているとみるのが正確。**現状これが実害にならない理由**は、Gemini自体にcold start概念がなく、同時に2件のGemini呼出しがあっても致命的な問題を起こしていないため。一方PaddleOCR(Cloud Run、`--concurrency=1`)ではtick重複時の2文書目以降が必ず新規インスタンスのcold start(実測27.9〜51.3秒)を踏むため、**同じ現象がPR6後に初めて実害化する**。`concurrency=1`明示設定の要否・サイクル所要時間の監視指標化は、PR6のロールアウト計画で扱う(Issue #966参照)
    - **min-instances=1の効能(上方修正、ただし条件付き)**: **tick重複が起きていない限り**、現状の実トラフィックは完全に直列(1サイクル内で次のリクエストは前の完了を待つ)であるため、min-instances=1は「バーストの1件目のみ」ではなく実質的に**ほぼ全リクエストのcold startを解消する**(tick重複時の2件目以降、およびインスタンス入替・新revisionデプロイ直後は引き続きcoldになりうる)。decision-maker判断により恒久採用した(`deploy-paddle-ocr.yml`)。常時1インスタンス分のCloud Run課金が発生する(下記「コスト監視」節参照)。
