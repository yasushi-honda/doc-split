...
既存のマスターデータをCSVとして書き出し、Excel等で編集して再インポートすることで一括修正できます（Issue #1036）。

**エクスポート手順:**
1. マスター画面 → 対象タブ
2. 「エクスポート」ボタンをクリック（登録データが0件のときは無効化されます）
3. ダウンロードしたCSVをExcel等で編集
4. 「CSVインポート」ボタンから編集後のCSVを再取込

**顧客・事業所マスターのID列:**
- 顧客・事業所マスターのエクスポートCSVには、先頭に`id`列が付きます
- `id`列の値がある行は、名前を変更しても「同じ人・同じ事業所」として更新されます（名前以外の列は変更を反映、名前列自体はCSVで書き換えても既存の名前のまま維持されます）
- `id`列の値は他の行にコピー・移動しないでください。誤って別の行のIDを使うと、意図しないレコードが上書きされます
- 書類種別・ケアマネマスターにはID列がなく、従来通り名前で同一レコードかどうかを判定します

**空欄セルの扱い:**
- 上書きインポート時、CSVで空欄のセルはその項目を変更しません（既存の値がそのまま残ります）
- 値を削除したい場合はCSVではなく、マスター画面の編集ダイアログを使ってください

**別表記（エイリアス）列の注意:**
- 別表記はCSVに値があれば、その内容で別表記リストを直接置き換えます（追記ではなく置換）
- 別表記は書類のOCR結果からマスターを自動判定する際に使われるため、変更内容をインポート前のプレビュー画面でよく確認してください
