...
| 項目 | 確認方法 | 頻度 |
|------|----------|------|
| 全体稼働状況 | [健全性レポート](health-report.md)（メール自動配信） | 毎日 09:00 JST |
| 書類処理状況 | 書類一覧画面 | 毎日 |
| エラー発生 | エラー履歴画面 | 毎日 |
...
4. 次回ポーリング（1分以内）でOCR処理が開始

### OCR処理のタイミング

| 処理方式 | タイミング | 用途 |
|---------|-----------|------|
| **スケジューラ（ポーリング）** | 1分間隔 | 全書類処理の唯一のエントリポイント（ADR-0010） |

**注**: アップロード後、最大1分以内にOCR処理が開始され、通常1〜2分以内に完了します。transientエラー（429/503等）は自動リトライされます（最大3回）。
...
DocSplitはPWA対応のため、スマートフォンやPCのホーム画面に追加してアプリのように利用できます。
...
ホーム画面から起動するとアドレスバーなしのスタンドアロン表示になります。
