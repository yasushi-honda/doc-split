...
- **契約終了の利用者も表示**（一時切替、Issue #1033）: 既定は非表示（確認済み書類のみ）。全ユーザー共通の既定値はマスター管理画面（6.）で設定、この画面での操作は保存されない一時上書き
- **PDFアップロード**（Issue #1031）: ローカルPDFの手動アップロード。バックグラウンド化されており、モーダルを閉じても処理は継続、進捗・完了・要確認はトースト通知で表示
...
- 各マスターのCRUD操作
- CSV一括インポート（エイリアス列対応）
- **CSV一括エクスポート**（Issue #1036）: 既存データをCSVで書き出し、編集して再インポートする一括修正の起点。顧客・事業所は先頭にID列付き（名前を変更しても同一レコードとして更新される）
- テーブルビューで許容表記を一覧表示
- **契約終了利用者の表示既定値トグル**（顧客マスター、Issue #1033）: 「書類画面で契約終了の利用者を表示する」を全ユーザー共通の既定値として設定。個々のユーザーは書類一覧画面（2.）側で一時的に上書き可能
