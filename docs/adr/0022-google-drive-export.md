...
**根本原因の修正（Issue #811、2026-08-27）**: 初版の検索クエリは`trashed=false`固定だった。ユーザーが手動でフォルダをゴミ箱に入れると、次回のエクスポート時に「0件マッチ」と誤判定され新規フォルダが作成され続け、同名フォルダの物理重複を生んでいた（kanameoneの「森奈穂美」ケアマネフォルダで6重複が発生し、241件のdocumentがtrashed配下に取り残される実害が確認された。データ移行はPhase B Part Aの専用スクリプト群で実施済み）。あわせて、1ページ目のみで打ち切っていた`files.list`のページネーションも全ページ列挙に修正した。

検索は**2段階**で行う（初版は無条件でtrashed込み検索していたが、kanameone本番デプロイ直後に別の顧客配下「報告書」フォルダで回帰を起こしたため即日訂正、詳細は次段落）。まずactiveのみ（`trashed=false`）で検索し、1件ならそのidを即座に返す（trashed側は一切検索しない）。active 0件の場合のみtrashed込みで再検索し、1件なら`files.update({trashed:false})`で復元してから再利用する。いずれの段階でも2件以上見つかった場合は`AmbiguousFolderError`で停止する。復元APIが失敗した場合は新規作成へフォールバックせず例外を再送出する（同名フォルダが実在するため、フォールバックは新たな重複を作りかねない）。
