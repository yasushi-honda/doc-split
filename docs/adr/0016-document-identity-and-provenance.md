...
3. **MUST NOT**: `rotatePdfPages` は **既存 Storage object path への上書き** および **callable 内での旧 path 削除** を行わない。回転結果は必ず新しい canonical object path (例: 新 docId namespace、または同 docId 配下の新 fileName) に書き込み、Firestore の `fileUrl` / `provenance` (`derivedObjectPath` / `derivedGeneration` / `derivedSha256` 等) を新 object に更新する。旧 object の削除は番号認可付き destructive migration / 明示 `deleteDocument` 経路でのみ実施する。
...
1. **SHOULD**: 既存 docs への backfill (PR-D4) は best-effort 方針:
   - 親 PDF 現存 + sha256 計算可能 → `provenance` 全 fields 書込
   - 親 PDF 不在 or sha256 計算不能 → `provenance` 書込なし (PR-C3 の自動復旧 gate で Ambiguous 降格対象として処理)
...
| **PR-D2** | `splitPdf` 改修 (新規 split で provenance 10 fields 書込 + `derivedObjectPath` canonical 化)。impl-plan 前提: `file.save()` + 後続 `file.getMetadata()` で子 object の `derivedGeneration` / `derivedMetageneration` を取得 (2 API call、許容範囲。Codex efficiency Low 1) | forward-only (新規 split のみ影響) | impl-plan + 実装後の 2 段階で必須 |
| **PR-D3** | `rotatePdfPages` 改修 (in-place 編集禁止 + 新 path / 新 generation 書込 + 安全な delete 経路) | forward-only (新規 rotation のみ影響) | impl-plan + 実装後の 2 段階で必須 |
| **PR-D4** | 既存 docs の provenance fields backfill (best-effort、可能な範囲のみ)。impl-plan 前提: 親 PDF 平均サイズ × doc 数 = GCS egress GB + Cloud Functions CPU 秒のフェルミ試算必須。concurrent N=8 を上限の初期値とし、kanameone 5,725 docs + cocoro 539 docs の総コスト試算を PR-D4 impl-plan で記載 (Codex efficiency Medium 1) | **destructive (既存 docs への書込)。MUST: 番号認可付き実行 + dry-run + dev リハーサル 7 stages × 2 周 (PR-C3c プロトコル準拠) + Codex MCP impl-plan/実装後 2 段階 review** | impl-plan + 実装後 + dev リハーサル 7 stages × 2 周 (PR-C3c プロトコル準拠) |
