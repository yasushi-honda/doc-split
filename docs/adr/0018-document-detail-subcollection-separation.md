...
| **E ✅main統合完了(2026-07-10、session112。本番`--execute`は未実行)** | **2026-07-10 Amended反映: dual-write停止をPhase Eへ前倒し統合、PR-E1/PR-E2の2段構成に変更**(当初計画は削除のみだったが、削除完了後も新規処理・再処理のたびに本体へ値が復活し「Phase E完了=egress実削減発生」の前提が崩れることをCodexセカンドオピニオンで検出)。**PR-E1**: 表#1,2,4,7,8,11の書込み箇所から本体への値set/update書込みを除去(`getReprocessClearFields()`の`deleteField()`は安全策として維持、対象外)。`ocrProcessor.ts`/`pdfOperations.ts`(splitPdfのparentPayload/detailPayload分離)/`checkGmailAttachments.ts`/`uploadPdf.ts`/`import-historical-gmail.js`/`seed-dev-data.ts`改修 + 書込み側契約テスト新設。**PR-E2**: migration marker機構(`_migrations/adr0018PhaseEPreflight`、PR-E1デプロイ未確認環境での誤実行を防止) + `documentGroups`トリガーへのdiff最適化(`isAggregationUnchanged()`、集計キー不変時は下流write 0) + 削除スクリプト(`scripts/delete-legacy-ocr-fields.ts`、状態別5分類ロジック`decideDeletionAction`、削除直前canonicalHash同一tx内再検証、Firestoreのみのmanifest記録) + `--rollback`(manifest限定復元、将来write不可逆) + FE契約テスト新設(`detailReaderCutoverContract.test.ts`) + FE既知リスク2件修正(`PdfSplitModal`のdetailLoading/detailErrorゲート、`DocumentDetailModal`のisError表示)。**egressの実削減効果はPR-E2の`--execute`実行で初めて発生**(Firestoreはprojection非対応のため、フィールドを実際に本体から消さない限り一覧クエリの転送量は変わらない)。トリガーストーム対策: documentGroups diff最適化 + レート制御(`runWithRateControl`、Phase C同様CONCURRENCY=4/150ms間隔)の二重対策 | **Yes(destructive、番号認可+dry-run+devリハーサル必須。実装はimpl-plan承認済み、本番`--execute`実行は別途番号認可)** | impl-plan(session111、Codex 2周) + 実装後review + devリハーサル |
