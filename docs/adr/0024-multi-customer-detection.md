...
**検証済み（2026-09-15追記）**: Stage2切替後、実際に新しい複数人記載FAXが到着した際の最終挙動確認（複製されず検出バッジのみになること、Cloud Loggingで`reason: flagDisabled`になること）を、8/30時点から約17日経過した2026-09-15に確認した。`audit-fax-duplication-inventory`再実行で`detectionOnlyCount`（検出はされたが複製されなかったdoc数）が0件→424件に増加（複製された91件/20グループは不変、想定通り）。あわせて`faxDuplicationPlan`ログ（過去504時間、GHA `audit-fax-duplication-plan-logs`スクリプト経由）で`reason: flagDisabled`が3,669件観測された。Stage2後に到着した新規複数人記載FAXが「複製されず検出のみになる」設計通りの挙動を示すことを本番データで確認できた。
