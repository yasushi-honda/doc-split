...
- [x] **事後監視のpush型アラート強化(2026-09-19)**: 手動ワンショット確認(pull型、人間の記憶依存)の弱点をFable 5.1セカンドオピニオンで指摘され、Cloud Monitoringアラート(push型)をkanameone/cocoroに追加(PR: `monitoring/paddle-ocr-post-switch-alerts`)。`processocr_error`(processOCR document処理エラー、閾値1件/1h、`lifecycle:temporary`/`review_by:2026-10-03`)を両環境に適用済み。当初追加したlatency p95アラートは、processOCRが1分間隔・concurrency:1で1 tick内に最大5件を直列処理する設計のため「1 tickの所要時間」であって「PaddleOCR呼び出しのlatency」ではなく、5分windowのp95が実処理tick混入で日常的に誤発火する設計不備がFable指摘で判明し、適用直後(cocoro/kanameone)に撤回済み。正しい設計は別Issue([[reference_processocr_latency_alert_design]]、GitHub Issue #979)で再検討。fail-open(list系コマンドの権限エラー握り潰し)の構造的懸念はGitHub Issue #978で別途起票
- [ ] **次の一手**: 切替後の事後監視(手動ワンショット、push型アラートの補完として継続)。各環境で処理件数が一定数集まった時点でstatus:error率・Cloud Run latency(p95/60秒超過率)をStep0④実測ベースライン(kanameone: total=1949/7日・error率0%・p95=0.42s・60秒超過0.29%、dev/cocoroも同様に記録済み)と比較し、悪化していないか確認する。運用コスト実測・抽出精度統計検証(1-2週間のデータ蓄積待ち、クライアント報告用)は切替の前提ではなく並行継続中([[feedback_data_collection_window_vs_safety_gate]])。ADR-0025本文へのPass1全面切替実施の追記は別タスクとして残る。2026-10-03(review_by)までにprocessocr_errorアラートの削除/恒久化を再判断すること
