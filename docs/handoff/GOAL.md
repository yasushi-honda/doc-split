...
**Phase B-1〜B-3完了(2026-07-18、feature branch `issue-680-phase-b-tokenizer`)**。`functions/src/utils/tokenizer.ts`の`generateFieldTokens()`に`includeBigrams`オプションを追加し、fileNameトークン化を`extractFilenameInfo()`のprefix経由に変更、`prefixType`(`office_name`のみbigram、`phone_number`/`document_id`/`unknown`はkeywordのみ)で制御する設計を実装。tokenizer.test.tsにAC-B1〜B4対応の新規テスト8件を追加し全PASS(functions全体1848件PASS、tsc型チェックエラー0件)。Firestore emulator経由でsearchDocuments/searchIndexer統合テスト20件もPASS(回帰なし)。ts-node手動確認でfax様3パターン(`DOC260718-L1-...`/`0529088423-L1-...`/`西春内科在宅クリニック-L1-...`)全てで原因トークンID(`00000644`="26"、`00000d45`="l1")が非生成であることを直接確認。

次はB-4(`/code-review`実施)着手待ち。急ぎではない(Phase Aで根本のFirestoreインデックス上限エラーは既に解消済みのため)。B-5(本番デプロイ)はB-4完了+decision-makerの明示認可が必要。
