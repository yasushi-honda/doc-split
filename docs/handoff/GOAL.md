...
- [ ] G. dev環境A/Bテスト実行（4項目の精度・トークン増分・grounding失敗率確認、D,E,Fに依存。タスクFのハーネスで一次スクリーニング済み=baseline/candidate両方4項目100%一致・N=5、本実行では対象文書数を拡大して再現性を確認する）
