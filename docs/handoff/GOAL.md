...
- [x] **候補調査**: WebSearch/WebFetchで2026-09-21時点の情報を収集。Qwen3.5-4B/Gemma4 E4B/llm-jp-4-8b-instruct/Sarashina2.2-3B/PLaMo 2 8B/ELYZA-Diffusion-Instruct-1.0-Dream-7B等を比較。ライセンス・ベンチマーク数値は可能な限り一次ソース(HFモデルカード・LICENSE本文・公式ブログ)で本人が直接検証(調査エージェントの報告を鵜呑みにせず、llm-jpのベンチマーク数値の食い違い・GitHub issueの誤引用等、複数の誤りを発見・訂正した)
- [x] **実機検証・1次(dev、Cloud Run 8vCPU/32GiB、CPUのみ)**: 3候補(Sarashina2.2-3B/Qwen3.5-4B/llm-jp-4-8b-instruct)をGGUF量子化(Q4_K_M)でデプロイし、架空の模擬介護文書4種(FAX送付状〜居宅サービス計画書8,000字)で本番同等プロンプトを実行。**Qwen3.5-4Bは実在しない年齢(「42歳」等)を3/3回再現性ある形で捏造**しており不採用。llm-jp-4-8b-instructは精度は高いが長文で700秒超と遅すぎる。この段階ではSarashina2.2-3B(Q4_K_M)は捏造0件と判定したが、下記の2次検証(弱点を突く追加書類)で誤りと判明した
- [x] **プロンプト改善(v2)**: Sarashina2.2-3Bの長文(8,000字)でのカバー率が56%(必須項目の脱落)だった問題に対し、必須5項目(種類/関係者/日付/金額/状態変化)を明示し箇条書き形式に変更するプロンプトへ改善、再検証でカバー率94%まで改善。ただし行数指定(5〜7行)は3Bモデルでは守られず10〜14行になる副作用あり
- [x] **実機検証・2次(2026-09-21、decision-maker指示「実用の検証を可能な限り工夫して試行錯誤」を受けて実施、弱点を突く追加模擬書類5種+量子化/温度の比較)**: D1〜D4(1次)は全て実在の事業所名を含んでいたため見えていなかった弱点を、意図的に情報が欠落した書類で洗った。
  - **重大な発見(Q4_K_M不採用の決め手)**: 事業所名の記載が一切ない正式書式の書類(D9、福祉用具貸与確認書)で、**Sarashina2.2-3B(Q4_K_M)が実在しない事業所名「介護サポート株式会社」を、温度0.2/0(決定論的)の両方で合計8/8回、完全な再現性をもって捏造した**。Qwen3.5-4Bの年齢捏造と同型の欠陥(情報欠落箇所をもっともらしい固有名詞で埋める)。一方、体裁のない単なるメモ(D10、固有名詞ゼロ)ではQ4は5/5回とも正直に「記載なし」と回答しており、**捏造は常時発生ではなく「正式書類の体裁+一部項目の欠落」という実運用で典型的な条件で誘発される**点が重要(発見の経緯は自作の`ORG_PAT`正規表現スキャナで全result_*.json横断走査、誤検出多数の中から本件のみ実在するソースに存在しない組織名と確認)
  - **Q8_0(高精度量子化)は同条件で捏造なし**: D9をQ8_0で温度0.2(5回)・温度0決定論的(3回、追試)の計8回とも「事業所名（省略/詳細不明）」と正直に回答。D10(固有名詞ゼロ)も5/5回で問題なし。D8(複数利用者・複数日付・複数受診先が混在する合同報告書)でも対象者ごとの受診先・日付の対応関係の取り違えなし(5回中5回でプログラムによる基本事実の整合確認、うち3回は出力全文も目視確認済み)。D5(金額記載なし)・D6(類似数値多数)・D7(OCR劣化模擬)はQ4/Q8とも問題なし
  - **速度のトレードオフ(短文D5〜D9限定)**: Q8_0はQ4_K_M比で約1.5〜2倍遅い(例: D8で22秒→41秒)。低頻度・非同期処理という用途では許容範囲と判断
  - **結論(1次時点)**: 採用するなら**Q8_0量子化**(Q4_K_Mではない)。速度より正確性(固有名詞の非捏造)を優先する
- [x] **実機検証・3次(2026-09-21、「品質は実用レベルか」との質問を受け、1次・2次で未実施だったQ8_0×実運用に近い長さの書類(D1〜D4)の組み合わせを追加検証してから回答)**: 1次(Q4_K_Mのみ)・2次(Q8_0だがD5〜D10の短文のみ)のいずれもQ8_0×D1〜D4(最大8,000字)の組み合わせが未検証のまま欠落していた盲点を埋めた。
  - **速度の訂正(重要)**: D1〜D4ではQ8_0とQ4_K_Mの速度差はほぼ無い(例: D3長文で166秒→168秒、D1短文で28秒→31秒)。上記「速度のトレードオフ」節の1.5〜2倍遅いという結論はD5〜D9(短文)限定の傾向であり、実運用に近い長さの書類には当てはまらないと判明。Q8_0を選ぶことに実質的な速度コストはない
  - **カバー率**: D1=100%、D2=86%、D4=90%、D3(長文)=89%(いずれもプロンプトv2、3回平均)。基準のGemini 3.5 Flash自体も57〜100%(D4が最低)であり、同水準
  - **固有名詞の捏造**: `ORG_PAT`正規表現スキャナで全出力を再走査し0件(検出された候補は全て地の文を巻き込んだ誤検出と確認済み、出典に実在する語句)
  - **残る欠落パターン(捏造ではなく省略)**: D4で「慢性心不全」「要介護4」のような分類・状態区分情報が稀に抜ける(捏造ではなく省略、実害は情報の欠如であり誤情報ではない)
  - **総合判定**: 「実用レベルか」という問いへの回答は条件付きでYes。低頻度・人が結果を見る用途であれば試す価値のある品質に達しているが、無条件の太鼓判ではない。未検証のまま残る点: ①実際のPaddleOCR誤読パターン(簡体字置換等)での検証は模擬ノイズのみで未実施 ②試行回数はまだ数十回規模で低頻度欠陥を排除できない ③実在の本番書類(匿名化・許可済み)では一度も検証していない ④同時実行・負荷状態は未検証。実装する場合は品質懸念時にGeminiへ切り替えるフォールバック設計を含めることを推奨
- [x] **ライセンス確認**: `sbintuitions/sarashina2.2-3b-instruct-v0.1`(および基盤モデル`sarashina2.2-3b`、量子化元は変わらないためQ8_0にも同一適用)はMIT、LICENSE本文(著作権者SB Intuitions 2025)まで直接確認済み。ゲート制限なし、商用利用・複数クライアント環境への配置とも問題なし。**同ブランドのTTS版(`sarashina2.2-tts`)のみ別ライセンス(非商用)のため混同注意**。モデル自体の公開日は2025-02-26(既存モデル、2026年の新着ではない。直近更新2026-08-25)
- [x] **追加候補の確認(PLaMo 2 8B/ELYZA-Diffusion-Instruct-1.0-Dream-7B)**: いずれもSarashinaより明確に優位とは言えないと判断。PLaMo 2 8Bは「PLaMo Community License」(年商10億円以下は無償だが利用登録フォーム提出必須、再配布にライセンス波及制限)でMITより制約が強く、サイズも3Bの約2〜3倍でCPUではより遅い。ELYZA-Diffusion(拡散言語モデル、Apache 2.0)は日本語ベンチマーク数値がモデルカードに記載なく未検証、CPU実測情報もなし(いずれも同種の固有名詞捏造テストは未実施)
- [x] **ROI判断・棚上げ(2026-09-21)**: 要約は低頻度・手動実行のみでGemini依存脱却の締切圧力がOCR(Vertex AI東京リージョン非公式・Gemini 2.5廃止)と異なり現状ない。上記2次検証によりSarashina(Q8_0)は実用に耐えうる客観的根拠(固有名詞捏造テスト複数種でPASS)を得られたため候補としての確度は上がったが、PLaMo/ELYZA等の追加検証は、現時点で判明している条件(ライセンス制約・サイズ・未検証)だけでSarashinaより不利な要素を抱えており、深掘りしても「Sarashinaを超える」結論に届く見込みが低いと判断し、decision-maker合意のうえ調査を打ち切った
- [x] **アーキテクチャ・コスト再検討(2026-09-21、同日追加セッション)**: decision-maker指摘「お客様目線で数分待たされる体験はあり得ない」を受け、UI設計を「明示クリック→同期待ち」から「OCR完了イベント駆動の非同期自動生成」へ転換。以下、この転換を前提に再検討した内容。
  - **Cloud Run運用方針**: min-instances=0(サーバーレス、コールドスタートあり)を採用。非同期バックグラウンド処理のため、コールドスタートはユーザー体験に影響しない(OCR本体=PaddleOCRがmin-instances=1でウォーム化しているのは、Functionsの同期タイムアウト制約を受けるためであり、要約には同じ制約がないことを整理して区別した)
  - **コールドスタート実測(Q8_0、dev、8vCPU/32GiB)**: `ready_wait_s`(スケールゼロからサービス起動+モデル読込完了まで)=65.9秒、`cold_first`(準備完了直後の初回リクエスト、D1短文)=27.88秒 → 合計約94秒。長文(D3相当)はウォーム状態処理時間(166〜168秒)から近似して約232〜234秒
  - **実データ集計(read-only、`inspect-ocr-volume-stats`ワークフロー、GitHub Actions経由)**: 直近30日processed文書数 kanameone 4,701件・cocoro 268件・dev 9件(テスト環境、無視可)。合計約4,969件/月(≈166件/日)。これを要約呼び出し回数の近似値として使用
  - **コスト試算の変遷と訂正(教訓として記録)**: ①当初min-instances=1(常時ウォーム)前提で試算し月$300〜490という結果を出したが、これは「非同期処理なのでウォーム化不要」という設計方針そのものと矛盾する誤った前提だった(decision-maker指摘「Sarashinaはコールドですよね？何故ウォームが前提？」で発覚・訂正)。②min-instances=0(全リクエストが毎回コールドスタートを踏む前提)で再計算: 月$127〜316。③定期ポーリング化(コールドスタート回数を実質日次1回程度に集約、詳細は次項)で再計算: 月約$103まで圧縮。**同種の試算を再度行う際は、「常時稼働前提」と「イベント駆動非同期・コールドスタート前提」を混同しないよう、試算前にアーキテクチャ前提(min-instances値)を明記してから計算すること**
  - **アーキテクチャ方針(確定)**: Cloud Run Jobsは不要。既存の通常Cloud Runサービス+Cloud Scheduler定期ポーリング(30分〜1時間間隔、PaddleOCR側の`processOCR`と同じ`onSchedule`パターンに準拠)で実装する。ポーリングのたびに「未処理(要約未記入)件数」をFirestoreで軽量チェックし、0件ならSarashinaへのリクエスト自体を送らない(コストゼロ)。件数が多い場合も「朝までに埋まればよい」程度の緩い制約で足り、1回のジョブで全件を処理しきる必要はない(バックグラウンド処理のためユーザーストレスは生じない)
  - **コスト再試算(ポーリング化・実測ベース)**: ページ数分布(kanameone 1ページ68.1%/2ページ以上31.9%、cocoro 1ページ40.1%/2ページ以上59.9%)から短文(30秒)/長文(167秒)の加重平均処理時間≈75.8秒/件を算出。コールドスタートは日次1回(30日×65.9秒)のみ負担する前提で合計約378,528秒/月(CPU active+Memory active)。**月額概算 約$103(≈15,500円)**。Gemini非公式PayGo全件自動化(約$100〜110/月)とほぼ同水準。待機課金なし(min-instances=0)のため、処理量が0の時間帯は完全に無課金
  - **手動アップロード時のOCR処理フロー確認(Explore agent調査、実コード確認済み)**: `uploadPdf`(Callable Function、`functions/src/upload/uploadPdf.ts`)はStorage保存+Firestore `documents(status:pending)`作成までのみ`await`し、OCR実処理(`processOCR`、`onSchedule`・1分間隔ポーリング)は完全に別プロセス・ユーザー操作から独立して実行される。アップロードボタンのローディング解除はOCR完了を待たない。要約もこれと同じ非同期パターンに倣う設計とする
  - **データ委託構造・コンプライアンス整理**: GCPプロジェクトの契約主体はクライアント自身であり、doc-split運営者はその環境を借りてシステムを構築する立場(Vertex AI利用時もCloud Run自前ホスティング時も共通、ここは変わらない)。インフラ責任(可用性・パッチ・物理セキュリティ)もどちらのケースでも引き続きGoogleが担う。**変わるのはアプリケーション層の責任のみ**: Vertex AI利用時はGoogleの完成品AIサービス(Gemini)の精度・品質をGoogleが担保するが、Cloud Run自前ホスティング時はモデル(PaddleOCR/Sarashina)の選定・実装・チューニング・保守をdoc-split運営者が引き受ける。データの渡し先も、Vertex AI(クライアントのプロジェクトから呼び出すが実際の推論はGoogle運営の別レイヤーのマルチテナント推論基盤で行われ、東京リージョン指定は非公式)から、Cloud Run内(クライアントのプロジェクト内に専有デプロイされたコンテナで完結、リージョン指定は公式サポート)に変わる。これが東京リージョン非公式利用リスクを解消する技術的な核心。「Cloud Runの方がセキュア」という理解は不正確(技術的な暗号化・アクセス制御水準はVertexAI非公式利用でも変わらない)で、正確には「処理場所を自分たちで保証できるようになる」「委託先(Google Vertex AIへの再委託)が1つ減り契約管理がシンプルになる」という効果
  - **総合結論**: コストだけで見れば現行Gemini非公式PayGoとほぼ同水準〜わずかに高い可能性が残るが(数千円〜3万円程度/月の差)、移行の主目的はコスト最適化ではなく東京リージョン非公式利用の調達継続性リスク回避(公式移行先はProvisioned Throughput、月$2,200〜3,000)。この程度の差なら速度・精度・リスク回避を優先すべきというdecision-maker判断により、追加のスペック最適化検証(vCPU数を減らす等)はROI的に見送り
- [x] **次の一手(トリガー付き) — トリガー②が発生し再着手決定(2026-09-21)**: 「②要約機能をSLMへ実装移行する明示決定が出た場合」に該当し、decision-maker承認によりplan modeでの実装計画策定に着手する。再着手時は、本節の暫定候補(**Sarashina2.2-3B・Q8_0量子化**、プロンプトv2、下記テンプレート)と上記の確定済みアーキテクチャ方針(非同期ポーリング・min-instances=0・Cloud Run Jobs不要)を起点にしつつ、①固有名詞捏造テスト(D9/D10相当、情報欠落パターン)を実装前の必須ゲートとして継続する ②PLaMo 2 8B(登録手続き含む)・ELYZA-Diffusionの実機検証(同じ捏造テストを含む)も改めて検討する ③実装時は本番コードの変更(`summaryPromptBuilder.ts`・feature flag・Cloud Runサービス新設・Cloud Scheduler設定等)を伴うためCLAUDE.md CRITICAL(5ファイル以上/新機能/アーキテクチャ判断)に該当し、plan modeでのフル計画とdecision-maker承認が必須。検証用リソース(Cloud Run services `slm-bench-*`、Artifact Registry `slm-bench`)は毎回検証後にdevから削除済み、恒久リソースは未作成
- **改善版プロンプト(v2、本番`summaryPromptBuilder.ts`には未反映、再検討時の起点用に記録)**:
  ```
  あなたは介護関連書類の要約担当です。以下は「{doc_type}」のOCR結果です。
...
## 【進行中・2026-09-12開始】ADR-0025 PaddleOCR: dev/kanameone/cocoro全3環境でインフラ・L1配線・実文書canary完了、Stage3負荷試験ハーネス実装・Phase B実測実行とも完了（2026-09-18）、**必須ゲート(71ページ)PASS、Pass1切替(PR6)のNo-Go要因なし。1ページcoldゲートFAILはmin-instances=1で緩和(PR #967)。processOCRのtick重複(60秒境界超過時)による複数文書同時OCRリスクはPR6のロールアウト計画で扱う**（現在のミッションとは別件・並行トラック）
...
- [x] **タイムアウト安全マージン検討【完了・2026-09-15】**: 旧記述「540秒上限、安全マージン約15秒」は前提誤りと判明(540秒はADR-0010で廃止済みのデッドコード`processOCROnCreate`由来。実際の`processOCR`タイムアウトはADR-0023で540秒→900秒に引き上げ済み、`functions/src/ocr/constants.ts` `PROCESS_OCR_TIMEOUT_SECONDS=900`)。
  - 既存の`ocrProcessor.ts`フェーズ計測ログ(`event:'phaseTimings'`、Cloud Logging `textPayload`)から、非OCRオーバーヘッド(Storage download/Firestore書込み/candidate抽出Gemini呼出し/マスターデータ照合等)を実測。kanameone直近30日サンプル300件(1〜15ページ)では非OCRオーバーヘッド平均2.5秒/p95 3.4秒(最大outlier 12.2秒、コールドスタート起因と推定)
  - **実在する最大文書での実測(決定的)**: kanameone`documents`コレクションを`totalPages`降順でクエリし、実際に存在する最大文書(73ページ、`2ZJwJMjrz8ILE172BKRH`、2026-09-01処理、Gemini OCR時代)のphaseTimingsログを取得。`pageLoopMs`(OCR部分)284.2秒に対し**非OCRオーバーヘッド合計は9.9秒**(masterLoad 0.7秒・candidateGemini 1.6秒・customerMatch 3.7秒・officeMatch 3.3秒・date/tx/cleanup 0.6秒)。設計上の50秒マージンに対し実測は約1/5で、5倍の余裕を確認
  - `inspect-ocr-volume-stats.js`(GitHub Actions経由、read-only)で90日間の全期間totalPages分布を確認: 最大ページ数71(負荷ゲートのティアと一致)、51-100ページ帯は9885件中2件(0.02%)のみで極めて稀
...
- [x] **Phase B実行完了(2026-09-18 09:20〜15:20 UTC、5回のGHA dispatch)**: decision-maker承認(AskUserQuestion経由「承認済み計画通り全5回を順次開始」)を受け、承認済み順序(1p→71p warm→71p cold→20p→160p)で実行。実測値・全生ログはrun 35328962449/35330732094/35347105402/35348557755/35355523661、詳細な数値・測定条件・考察は`services/paddle-ocr/README.md`「PR4c実測値」節に記録済み。結果概要:
  - **71ページ(必須ゲート): PASS**(p95=600.5秒、基準850秒に対し約29%のマージン。ページ完了率100%、1,420/1,420リクエスト成功)。20ページもPASS(p95=173.5秒、基準400秒に対し約57%のマージン)。**Pass1切替(PR6)着手のNo-Go要因はなし**
  - **1ページのcoldゲートはFAIL**(coldMax=33.4秒 > 基準30秒、5バースト中1回目の外れ値、warm系列・ページ完了率は問題なし)。全4tierで「コールドバースト1回目のみ外れ値化(27.9〜51.3秒)」を確認。**バースト2回目以降(9〜10秒台)は真のcold再測定ではなく、cooldown(180秒)中に前回起動インスタンスがまだ温かいまま応答した疑いが残る**(fable-reviewレビュー指摘M2、`COLD_BURST_COOLDOWN_MS`の妥当性は依然未検証)。1ページは実運用の最頻ケース(57%)であるため、Phase Bの失敗判定表上は自動No-Goトリガーにならないが、緩和策の要否はdecision-maker判断が必要だった
  - liveness probe検知窓(300秒)は今回の1ページ実測(warm p95=9.26秒、cold最大33.4秒)を踏まえ、変更不要(300秒のまま確定)と判断・README反映済み
- [x] **decision-maker確定(2026-09-19、AskUserQuestion経由)**: (1) Pass1切替(PR6)着手は**Go**。(2) 1ページcold start変動はIssue #966として起票
- [x] **1ページcold start緩和策の追加スパイク検証+恒久採用(2026-09-19、PR #967)**: dev環境で`min-instances=1`に変更し、孤立した単発リクエスト(3秒間隔×3回)で外れ値解消(6.2〜6.4秒)を確認。一方、意図的な3件同時バーストでは外れ値(34.5秒)が継続。decision-maker承認により`deploy-paddle-ocr.yml`で恒久採用(`--min-instances=0`→`1`)。2026-09-19時点でdevのみ反映済み、kanameone/cocoroは次回dispatch時に反映(現状revision`00001`のままminScale=0を実機確認済み)
- [x] **PR #967誤記訂正、2段階(2026-09-19、PR #968+追加修正、fable-reviewを2回起用+自己検証)**: マージ後にPR6計画準備の調査で、PR #967に含めた「Gmail添付ポーリングが`processOCROnCreate.ts`経由で並行起動する」というバーストリスクの根拠が誤りと判明(`processOCROnCreate`はADR-0010で廃止済み・未デプロイのデッドコード、`gcloud functions list`で確認)。1回目の訂正PR(#968)で「バーストは起こらない」と結論づけかけたが、これも`/fable-review`独立検証(1回目)で**過剰訂正**と判明したため「現状(Gemini運用)は問題顕在化していないがPR6後にリスクが上がる」と修正しPR #968を作成。しかしこのPR #968自体のレビューでも`/fable-review`(2回目)により**さらなる不正確さ**を指摘された: ADR-0023の実測(71ページ文書のページループが319秒=4.5秒/ページ、Gemini)から、**tick重複自体はGemini運用の現在でも起こりうる**(多ページ文書が1サイクルの5件に含まれれば60秒を超過、43%の文書が複数ページ。ADR-0023記載のインシデント当日にも実際に観測済み)。正確な整理は「tick重複という現象自体は現在も起こりうるが、Geminiにはcold start概念がなく実害が顕在化していない。PaddleOCR(`--concurrency=1`)ではtick重複時の2文書目以降が必ず新規インスタンスのcold startを踏むため、PR6後に初めて実害化する」。この2度目の訂正内容をPR #968へ追加反映(README.md/Issue #966/deploy-paddle-ocr.ymlコメント/`processOCROnCreate.ts`ヘッダ)。min-instances=1の評価も条件付きで上方修正(tick重複が起きていない限り実質ほぼ全リクエストのcold startを解消)。**教訓**: 二次AIレビューの指摘を鵜呑みにせず実機・ADRで裏取りする姿勢自体は機能したが、自分の訂正判断も同じ基準で再検証する必要がある(訂正は1回で終わらない可能性を前提にする)
- [x] **PR6: tick重複防止(`concurrency:1`)+監視基盤、計画→実装→3環境デプロイまで完了(2026-09-19、PR #969マージ済み)**: plan mode(`~/.claude/plans/eventual-dreaming-wind.md`)で計画、`/fable-review`2回(当初のインメモリ再入防止ロック案が非expire設計でOCR永久停止しうるH2欠陥と判明→firebase-functions公式`concurrency`オプションを発見し`concurrency:1`1行追加へ再設計)を経て承認。実装後、code-reviewer/comment-analyzer→Fable(codex代替、H1: absenceアラートduration=600sが`PROCESS_OCR_TIMEOUT_SECONDS=900s`と数学的に矛盾し誤発火しうると指摘→1200sへ修正+回帰防止contract test追加)→quality-gate-evaluator(2回、実機gcloud確認込み)の多段レビューで、**実装者自身の完了報告がevaluatorの独立実機再検証で2回覆った**(1回目: 存在しない`npm run type-check`参照・devデプロイ未反映をAC文言側の不備と誤認→AC分離で解消。2回目: 「cocoro導入完了済み」と報告したものが実際は未作成、dev側もH1修正が実機に未反映のまま、という**事実と異なる完了宣言**をevaluatorが検出)。両方修正し3回目のevaluator評価(JSON実値ベース)でAPPROVE、CI PASS確認後squash merge。**教訓**: dry-run成功と本適用成功を混同しない、環境ごとの完了報告前に`gcloud ... describe`実機確認を徹底する
- [x] **PR6マージ後、3環境(dev/kanameone/cocoro)への安全策デプロイ揃え(2026-09-19)**: PR #969マージ直後はdevのみ`concurrency:1`/`min-instances=1`反映で、kanameone/cocoroは旧設定(`concurrency:80`・`min-instances`未設定)のままL1ゲート(`OCR_PROVIDER=paddle`)が有効という状態だった(ユーザーからの状況確認を受けて発覚)。`deploy-functions.yml`(kanameone/cocoro、`ocr_provider_override=paddle`/`gemini_model_id_override=code-default`を明示指定して既存L1状態を保持)+`deploy-paddle-ocr.yml`(kanameone/cocoro、`first_deploy=false`)を実行し、3環境とも`concurrency:1`・`min-instances=1`・`OCR_PROVIDER=paddle`(維持)を実機確認済み
- [x] **Step0③ Cloud Scheduler retryConfig確認(2026-09-19)**: `gcloud scheduler jobs describe firebase-schedule-processOCR-asia-northeast1 --location=asia-northeast1`をdev/cocoroで実機確認。両環境とも`attemptDeadline=900s`・`retryConfig`未設定(デフォルト挙動)で一致(devは前セッションで確認済み・再掲)。**kanameoneは`gcloud config configurations activate kanameone`後の呼び出しが`Reauthentication failed. cannot prompt during non-interactive execution`で失敗、systemkaname@kanameone.comの対話認証(decision-maker作業)待ちのため未確認のまま**。計画書§3の想定通り`retryConfig.retryCount=0`明示設定の要否は、kanameone確認後にまとめて判断する
- [x] **Step0④ 切替前ベースライン記録(dev/cocoro、2026-09-19)**: ローカルgcloud CLIトークン(`gcloud auth print-access-token`)でFirestore REST API `runAggregationQuery`を直接叩く方式(firebase-admin/ADCは`invalid_rapt`でreauth必要のため回避)。
  - **status:error率(直近7日、`updatedAt>=7日前`基準)**: dev: total 4件・error 0件。cocoro: total 37件・error 0件
  - **1サイクル所要時間分布(processOCRのCloud Run request latency、直近7日、`gcloud logging read`、サンプル上限10000件)**: dev: p50=0.19s/p95=0.30s/max=163.0s、60秒超過3件(0.03%)。cocoro: p50=0.18s/p95=0.29s/max=42.1s、60秒超過0件。**注**: 計画書§4は「phaseTimingsログから算出」を想定していたが、Cloud Run request latencyの方が「1サイクル(tick)所要時間」をより直接的に表す実測値のため代替採用(既存ログの後処理より正確)
  - **kanameone(代替指標のみ、2026-09-19)**: gcloud CLI/ADCとも`systemkaname@kanameone.com`の対話認証切れでブロックのため、GHA`run-ops-script.yml`(`check-function-error-logs --hours 168`、SA `GCP_SA_KEY_KANAMEONE`、run id 35412462348・success)でCloud Functions severity>=ERRORログ件数を代替取得。**直近168時間でERRORログは`onDocumentWriteSearchIndex`の9件のみ(`Memory limit of 512 MiB exceeded`)、`processOCR`関連のERRORログは0件**。ただしこれはFirestoreの`status:error`率でもCloud Run request latency分布でもないため参考情報止まり。kanameoneの完全なStep0④(status:error率・latency分布の実測)完了にはdecision-maker対話認証(`gcloud auth login --configuration=kanameone`)または`run-ops-script.yml`への専用オプション追加のいずれかが別途必要
- [x] **Step0②③④ kanameone分完了+3環境L2状態の横並び記録(2026-09-19)**: kanameoneのローカルgcloud対話認証(systemkaname@kanameone.com)失効はdecision-maker側の対話ログイン待ちにせず、既存の`check-function-error-logs`と同じSA(GCP_SA_KEY_KANAMEONE)非対話認証パターンを`run-ops-script.yml`に`check-paddle-ocr-step0`として追加(PR #973)し、GHA経由で解決した(実行可能なexecutor作業をdecision-maker専属アクションと誤認していたことをユーザー指摘で訂正)。fable-review(Fable 5.1)でH1(L2の「フィールド不在=制限なし」と「空配列=fail-closed全拒否」の区別欠落)・H2(Firestore REST APIのHTTPエラーがcurlのexit 0によりsilent failureになる)を検出・修正。kanameone実機での初回実行で③④のcompositeFilter集計クエリが実際に400エラーで失敗し、Firestore REST `runAggregationQuery`のJSON→protobufトランスコーダに「where句がcompositeFilter(2条件以上)の場合、`aggregations`キーを`structuredQuery`より後に置くと誤って400を返す」既知の癖があることを実機調査(cocoroへの直接curlで再現・キー順序変更で解消確認)で発見・修正し、2回目の実行で全項目成功。
  - **kanameone実測値**: L1 `OCR_PROVIDER=paddle`。L2 `paddleOcr=true`・`paddleOcrAllowlist=3件`(canary運用中、全面展開ではない)。Scheduler `attemptDeadline=900s`・`retryConfig`未設定(dev/cocoroと一致)。status:error率(直近7日) total=1949・error=0・rate=0.00%。latency分布 件数=10041 p50=0.16s/p95=0.42s/max=195.76s、60秒超過29件(0.29%)
  - **3環境のL2 allowlist件数横並び(2026-09-19、追加確認)**: dev: paddleOcr=true・allowlist=3件。kanameone: paddleOcr=true・allowlist=3件。cocoro: paddleOcr=true・allowlist=1件。**3環境ともcanary(少数固定docId)段階で、全面切替(`--remove`)はまだどの環境も未実行**
...
- [x] **cocoro全面切替完了・3環境全てのPass1全面移行が完了(2026-09-19)**: dry-run(GHA run 35423673144)で対象canary docId(`neUsEvOzmqgnkazWytWr`、canary検証で使用した1件と一致)を確認後、本実行(GHA run 35423777908)。`settings/features.paddleOcrAllowlist`フィールド削除・`paddleOcr:true`維持をFirestore REST直接取得で実機確認。**dev/kanameone/cocoroの3環境とも新規文書は全てPaddleOCR経由(allowlist制限なし)、Gemini依存のOCR Pass1は解消**
- [x] **事後監視のpush型アラート強化(2026-09-19)**: 手動ワンショット確認(pull型、人間の記憶依存)の弱点をFable 5.1セカンドオピニオンで指摘され、Cloud Monitoringアラート(push型)をkanameone/cocoroに追加(PR #980)。`processocr_error`(processOCR document処理エラー、閾値1件/1h、`lifecycle:temporary`/`review_by:2026-10-03`)を両環境に適用済み(実際の適用はローカルnamed config対話認証経由。専用の非対話workflow`.github/workflows/setup-monitoring.yml`(`MONITORING_SA_KEY_*`)が既存だったが未使用のまま完了、次回以降はこちらを使うこと)。二度のFable指摘で二段階の設計不備が判明し修正済み: ①当初のlatency p95アラート(標準メトリクスrequest_latenciesベース)はprocessOCRが1分間隔・concurrency:1で1 tick内に最大5件を直列処理する設計のため「1 tickの所要時間」であって「PaddleOCR呼び出しのlatency」ではなく、5分windowのp95が実処理tick混入で日常的に誤発火する不備→適用直後(cocoro/kanameone)に撤回、正しい設計は別Issue(GitHub Issue #979)で再検討。②`processocr_error`のlog-based metricフィルタに含めていた`severity="ERROR"`条件は、`console.error()`がfirebase-functions/logger未使用のためCloud Loggingで自動的にERROR severityへ昇格されずDEFAULTのまま記録される実測(同型ログ`Drive export failed for document`で確認)と矛盾し、アラートが構造的に一度も発火しない状態だった→フィルタから`severity`条件を除去し両環境で再作成、実データでの陽性検証材料(該当ログの発生実績)は直近30日とも0件のため未検証のまま。fail-open(list系コマンドの権限エラー握り潰し)の構造的懸念はGitHub Issue #978で別途起票
- [ ] **次の一手**: 切替後の事後監視(手動ワンショット、push型アラートの補完として継続)。各環境で処理件数が一定数集まった時点でstatus:error率・Cloud Run latency(p95/60秒超過率)をStep0④実測ベースライン(kanameone: total=1949/7日・error率0%・p95=0.42s・60秒超過0.29%、dev/cocoroも同様に記録済み)と比較し、悪化していないか確認する。運用コスト実測・抽出精度統計検証(1-2週間のデータ蓄積待ち、クライアント報告用)は切替の前提ではなく並行継続中([[feedback_data_collection_window_vs_safety_gate]])。ADR-0025本文へのPass1全面切替実施の追記は別タスクとして残る。2026-10-03(review_by)までにprocessocr_errorアラートの削除/恒久化を再判断すること。`setup-log-based-metrics.sh`は全テンプレートを適用する設計のため、devで`setup-monitoring.yml`を再実行するとprocessocr_error(temporary)も自動作成される点に注意(devには現状未作成)。`teardown-log-based-metrics.sh`は全メトリクス一括削除のみ対応、processocr_errorだけの個別削除は手動`gcloud alpha monitoring policies delete`+`gcloud logging metrics delete`が必要
...
- [x] `/code-review medium`（feature/drive-export-phase1ブランチ全体）でCONFIRMED 8件検出、重大度上位4件を修正（commit 86a9030）。①reprocess時のDrive系フィールド残存(useDocuments.ts + firestore.rulesのhasOnly追加) ②pending状態廃止によるクラッシュ時の永久滞留解消(絶対→exportingを単一トランザクション化) ③driveExportRunId所有権トークン導入(ocrRunGuard.tsと同型、並行実行時の状態上書き防止) ④appProperties(docSplitDocId)ベースのDriveファイル冪等性チェック(重複アップロード防止)。ADR-0022に状態遷移図(mermaid)追加。残り4件のうちnull fileDate/空careManagerでの空フォルダは下記「`/code-review high`残りCONFIRMED」節で解消済み。**gs://バケット不一致**（`exportDocument.ts`の`fileUrl`文字列置換がデフォルトバケット名前提で、過去の別バケット名形式を指す文書があれば失敗しうる潜在リスク）と**updatedAt共有によるスタック誤判定**（`driveExportScheduled.ts`が汎用`updatedAt`を流用しDrive Export専用のタイムスタンプでない設計ギャップ）は2026-08-29時点で未対応のまま残存（実害の観測報告なし、優先度低）
- [x] `/code-review`（bare、xhigh/recall効果、feature/drive-export-phase1ブランチ全体）で10角度並列レビューにより15件検出(10エージェントが大幅遅延の末全員応答、重複排除して差替報告)。新規発見最重要2件から着手: ①firestoreToDocument()へのDrive系5フィールドマッピング追加(#178教訓、frontend/src/hooks/useDocuments.ts) ②reprocess時のDrive孤児ファイル問題修正 — `driveFileId`を`getReprocessClearFields()`のクリア対象から除外し、`exportDocument.ts`に`resolveDriveFile()`を追加(driveFileIdがあればfiles.get→files.updateで移動/リネーム/内容更新、404ならappPropertiesフォールバック)。これにより誤配置(旧フォルダへの孤児ファイル残置)とstale content(内容不更新)の両方を解消。type-check/lint/unit1903/integration193/rules84/frontend364全PASS確認済み。残り13件(初回エクスポート時のTOCTOU競合等)のうち異なるdocId間の作成競合は下記「`/code-review xhigh`」節（line 315、`driveFolderLocks`fencing token）で解消済みと判明したが、残り12件前後は本文中に個別列挙が一切なく2026-08-29時点でも内容不明・判定不能のまま（着手するには`/code-review`の再実行が必要）
...
- **codex(GPT、PM視点)セカンドオピニオン**: 「ログ追加のみで完了は不十分」という decision-maker 指摘は妥当。優先順位は `fail-closed維持` → `観測性追加(並行実施可)` → `証跡・再現による原因切り分け` → `業務方針・設計の正式決定(plan mode)` → `段階導入`。本質は「誰が動かしたか」ではなく「フォルダ移動を許容する業務・プロダクト契約が未定義」であること。「フォルダは動きうる前提」への設計見直しは妥当だが、「動いたなら即座にDrive実体を正とする」への飛躍は危険(物理配置とアプリが保証する論理的所属は分離すべき)
- **恒久対応の設計方針(未着手)**: Drive UI上の手動フォルダ操作(新規作成・移動)を、①原則禁止してアプリ経由に統制する方向、②承認付き再同期ワークフローとして正式サポートする方向、のいずれを基本方針にするかを業務側と合意する必要がある。介護書類の誤配置コストの高さを踏まえるとcodexは①を推奨。**新機能・アーキテクチャ判断に該当するため、着手時はplan modeで正式設計すること**(CLAUDE.md CRITICAL準拠)
...
## 【修正・デプロイ完了、効果確認待ち・2026-09-09】kanameoneの請求内訳調査で判明: 無限スクロール自動再取得によるFirestore読み取り過多
...
**修正実装完了（2026-09-08、PR #890マージ済み・mainブランチ）**: `/plan-crossreview`(grip自白可視化+Codex独立診断2パス)でプラン段階のHigh 3件(IntersectionObserverとreset処理の競合・一括操作部分失敗時のキャッシュ整合性・フィルタ切替後の古いキャッシュ表示)を洗い出し反映。実装は「一覧を自動では絶対に再取得しない。ミューテーションが新しい値を知っている場合はキャッシュを直接パッチ、それ以外は`refetchType:'none'`でstaleマークのみ、『更新があります』バナー経由のユーザー起点リセットで1ページ目のみ明示的に再取得」という設計。`codex review --base main`を計5回実施（3周目で「反応的パッチの繰り返し」の兆候を検知し、個別対応ではなく更新検知の仕組み自体を単純化する設計転換を実施）、`pr-review-toolkit`4エージェント並列セカンドオピニオン、ローカルFirebaseエミュレータ+Playwright MCPでの実機検証（250件超のテストデータで4ページ分無限スクロール後40秒アイドル/フォーカス復帰でも自動全ページ再取得が発生しないことをTanStack QueryClient内部状態で直接確認、フィルタ切替・確認操作→バナー表示→リセットのフロー・reset中のスクロール競合を確認）を経てマージ。

**デプロイ完了（2026-09-09）**: `/deploy`（GitHub Actions「Deploy Firebase Hosting」経由）でkanameone(run 34283465593)・cocoro(run 34283576962)双方へHostingデプロイ完了、いずれも成功。本PRの変更範囲はfrontendのみ（Functions/Firestoreルール変更なし）のためHostingのみで反映済み。dev環境は既存の通りmainへのpush時のCI自動デプロイで反映済み。cocoroは今回の請求調査の直接対象ではないが、同一の共有フロントエンドコードのバグのため水平展開した。
