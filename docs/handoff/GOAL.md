...
## 【修正実装完了・デプロイ待ち・2026-09-09】kanameoneの請求内訳調査で判明: 無限スクロール自動再取得によるFirestore読み取り過多
...
**現在の状態**: dev環境(`doc-split-dev`)にはmainへのpush時のCI自動デプロイで反映済み。**kanameone環境へは未デプロイ**（コスト問題の発生元であり、効果確認のためにはここへのデプロイが必須）。

**次の一手**: `/deploy kanameone`でFunctions/Hostingをデプロイ → デプロイ後の動作確認 → 翌月（2026年10月）の請求額でFirestore読み取り数削減を確認。同型リスクとして`useGroupDocuments`（グループ別一覧）にも同様の構造が疑われるため、follow-up Issue #891を起票済み（水平展開要否は別途調査）。
