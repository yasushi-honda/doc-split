...
- [x] AC-B1: `generateDocumentTokens({fileName: 'DOC260718-L1-20260718131435.pdf'})`のトークンに`"26"`・`"l1"`・時刻断片を含まない(証明: tokenizer.test.ts新規ケース、PASS。加えてts-node手動確認で`generateTokenId('26')`=`00000644`・`generateTokenId('l1')`=`00000d45`がfax様3パターン全てで非生成であることを直接確認)
- [x] AC-B2: `phone_number`/`document_id`/`office_name`/`unknown`の4分類それぞれを個別にテスト(証明: tokenizer.test.ts、PASS)
- [x] AC-B3: `田中太郎_介護保険.pdf`は従来通りbigram込み(回帰確認、証明: 既存テストケース+新規回帰テストPASS)
- [x] AC-B4: prefix抽出失敗時(拡張子のみ・空文字)はエラーを投げず、かつ不正な空トークンも生成しない(証明: 境界値テストPASS)
...
- [x] B-1. `tokenizer.ts`修正(`generateFieldTokens()`に`includeBigrams`オプション追加、fileNameを`extractFilenameInfo()`のprefix経由に変更、`prefixType`別bigram制御)
- [x] B-2. `tokenizer.test.ts`に単体テスト追加(AC-B1〜B4対応、B-1に依存)
- [x] B-3. dev環境でemulatorテスト全PASS確認+fax様fileNameパターンの手動処理確認
- [x] B-4. `/code-review`(実コード変更、medium effort、findings 6件→CONFIRMED 2件のうち1件は当PR内で修正済み、詳細は中断点参照)
- [ ] B-5. kanameone+cocoroへfunctionsデプロイ(GHA)
- [ ] B-6. Issue #680クローズ+案3(dfガード)の別Issue起票要否をtriage判断(再開条件を明記)

## 🔄 中断点（in-flight）

**Phase B-1〜B-4完了・PR #684マージ済み(2026-07-18、main `49d1e59`、squash merge済み、feature branch削除済み)**。`functions/src/utils/tokenizer.ts`の`generateFieldTokens()`に`includeBigrams`オプションを追加し、fileNameトークン化を`extractFilenameInfo()`のprefix経由に変更、`prefixType`(`office_name`のみbigram、`phone_number`/`document_id`/`unknown`はkeywordのみ)で制御する設計を実装。tokenizer.test.tsにAC-B1〜B4対応の新規テスト8件を追加し全PASS。Firestore emulator経由でsearchDocuments/searchIndexer統合テスト20件もPASS(回帰なし)。ts-node手動確認でfax様3パターン(`DOC260718-L1-...`/`0529088423-L1-...`/`西春内科在宅クリニック-L1-...`)全てで原因トークンID(`00000644`="26"、`00000d45`="l1")が非生成であることを直接確認。マージ前に`/review`(GitHub PR単体レビュー)も実施、指摘事項なし(マージ可能な品質と判断)。

**B-4 `/code-review medium`結果(8 finder角度→verify 1-vote、findings 6件)**:
- CONFIRMED×2のうち1件は当PR内で修正済み: `extractFilenameInfo()`のoffice_name判定が全角カナ・漢字・ひらがなのみ対象で半角カナ(レガシーFAX機器のJIS X 0201制約由来)を検知できず、正当な事業所名がunknown型に誤分類されbigram検索を失うバグ。正規表現に`｡-ﾟ`を追加し回帰テスト7件を追加(functions全体1855件PASS、tsc 0件)。
- CONFIRMED×1(未修正・follow-up候補): `extractFilenameInfo()`の`-L\d+-`マッチが、FAX由来でない任意アップロードファイル名(`uploadPdf.ts`はAPI入力の`fileName`を無条件に受け付ける)に偶然`-L<数字>-`様の部分文字列が含まれる場合(例: `見積書-L1000-final.pdf`)、prefixが途中で切り詰められ検索インデックスから内容が脱落する。既存関数(OCR事業所名マッチングで軽微な影響)の挙動だが、本PRでtokenizer.tsが検索インデックス生成に直接使うようになったため影響範囲が拡大。Phase Bの設計スコープ外(fax gateway専用ロジックの前提が非fax由来ファイル名にも適用される既存の設計上の穴)のため、triage要否をB-6と合わせて次回判断。
- PLAUSIBLE×4(低優先度、対応不要と判断): `-L\d+-`のcase-sensitivity(fax gateway命名規則は大文字`L`固定と推測されるため理論上のリスク)/ローマ字のみの事業所名がunknown型になる(実データ上全件日本語表記のため低確率)/`prefixType==='office_name'`判定の3箇所重複(既存idiomの拡張、リファクタ価値は小さい)/tokenizer.tsへのfax固有ロジック混入(altitude、tokenizer.ts自体が既にドメイン概念を持つため深刻な層違反ではない)。
- REFUTED×1: `phone_number`/`document_id`のbigram抑制による部分一致検索喪失は、GOAL.md記載の設計意図(短い数字bigramの汚染源化を避ける)通りの意図的トレードオフであり、Codexセカンドオピニオン+decision-maker判断を経て確定済みの挙動。バグではない。

**2026-07-18セッション終了時点でB-5は未着手**。decision-maker判断: 「今回はここで終了し、後日判断」を選択(search_index肥大化の根本原因はPhase A(firestore.indexes.jsonのfieldOverride)で既に解消済みのため、Phase Bのbigram品質改善は緊急性なし)。次セッション開始時は、B-5着手前にdecision-makerの本番デプロイ明示認可を得ること。あわせてCONFIRMED未修正1件(非fax由来ファイル名の-L\d+-偶然一致)の扱い(追加対応/follow-up Issue化/現状許容)も次回判断。
