...
## 【進行中・2026-09-12開始】ADR-0025 PaddleOCR: dev/kanameone/cocoro全3環境でインフラ・L1配線・実文書canary完了、Stage3負荷試験ハーネス実装・Phase B実測実行とも完了（2026-09-18〜19）、**必須ゲート(71ページ)PASS、Pass1切替(PR6)のNo-Go要因なし。1ページcoldゲートFAILの扱いのみdecision-maker判断待ち**（現在のミッションとは別件・並行トラック）
...
- [x] **Phase B実行完了(2026-09-18 09:20〜2026-09-19 15:20、5回のGHA dispatch)**: decision-maker承認(AskUserQuestion経由「承認済み計画通り全5回を順次開始」)を受け、承認済み順序(1p→71p warm→71p cold→20p→160p)で実行。実測値・全生ログはrun 35328962449/35330732094/35347105402/35348557755/35355523661、詳細な数値・測定条件・考察は`services/paddle-ocr/README.md`「PR4c実測値」節に記録済み。結果概要:
  ...
  - **1ページのcoldゲートはFAIL**(coldMax=33.4秒 > 基準30秒、5バースト中1回のみの外れ値、warm系列・ページ完了率は問題なし)。全4tierで「coldバースト1回目のみ外れ値化(27.9〜51.3秒)、以降は9〜10秒に収束」という一貫した再現パターンを確認、ページ数に相関しないためCloud Runインスタンス起動自体の変動と推定。1ページは実運用の最頻ケース(57%)であるため、Phase Bの失敗判定表上は自動No-Goトリガーにならないが、**min-instances引き上げ等の緩和策の要否はdecision-maker判断が必要**
  - liveness probe検知窓(300秒)は今回の1ページ実測(warm p95=9.26秒、cold最大33.4秒)を踏まえ、変更不要(300秒のまま確定)と判断・README反映済み
- [x] **decision-maker確定(2026-09-19、AskUserQuestion経由)**: (1) Pass1切替(PR6)着手は**Go**。(2) 1ページcold start変動(Issue #966として起票済み)は優先度を追って判断する位置づけとし、PR6着手のブロッカーにはしない
- [ ] **次の一手**: PR6(Pass1切替の実装)に着手する。本番切替という影響範囲の大きいアーキテクチャ判断のため、CLAUDE.md CRITICAL(5ファイル以上/新機能/アーキテクチャ判断)に該当し、**plan modeでのフル計画が必要**(親計画`~/.claude/plans/shiny-knitting-flamingo.md`のPR6節を起点に、L2 Firestoreフラグ・allowlist段階導入等の既存ロールアウト設計と整合させる)
