...
- ✅ **Issue #625**（OCR結果Storageの孤児オブジェクト無制限蓄積）完了（2026-07-12 session117）。`/impl-plan`軽量モード承認→実装→Codexセカンドオピニオン（plan、MCP high）で当初設計の前提破綻（FE側`getReprocessClearFields()`が`ocrResultUrl`を事前削除するため機能しない）を検出、Storage prefix listingベースに設計変更→`/safe-refactor`（DRY指摘1件は見送り判断）→`/code-review low`でレースコンディション検出・修正→PR #629作成、CodeRabbit指摘（削除ループ内再検証不足）に対応→マージ→Issue #625自動クローズ
  ...
  - 守り（検出）で新規起票したIssue（着手はdecision-maker指示待ち）: #625（Storage孤児化、P1、reprocess毎に旧ocrResultUrlオブジェクトが無制限蓄積）・#626（Gemini浪費、P2、所有権チェックが最終tx時のみでsupersededされたrunがAPIコストを消費し切ってから破棄）

### 守り（検出）で新規起票したIssue（#539のcode-review副産物、着手はdecision-maker指示待ち）
- #620: NOT_FOUND(親doc削除)がsplitPdf/rotatePdfPagesの`isFirestorePreconditionFailure`で「二重split」と誤診断されるメッセージ精度問題
- #621: splitPdf新エラーコード(already-exists/aborted)がFE未対応（CLAUDE.md API境界確認ルール抵触、生エラー表示+キャッシュ未invalidate）
- #622: `splitPdfConcurrencyGuardContract.test.ts`のgrep contractがsplitPdf固有の配線を実質保証できていない(vacuous test risk)
