...
## 【調査完了・棚上げ・2026-09-21】要約生成(regenerateSummary)のGemini依存脱却: 自前ホスティングSLM調査 — Sarashina2.2-3Bを暫定候補として記録、実装は着手せず
...
- [x] **実機検証(dev、Cloud Run 8vCPU/32GiB、CPUのみ)**: 3候補(Sarashina2.2-3B/Qwen3.5-4B/llm-jp-4-8b-instruct)をGGUF量子化(Q4_K_M)でデプロイし、架空の模擬介護文書4種(FAX送付状〜居宅サービス計画書8,000字)で本番同等プロンプトを実行。**Qwen3.5-4Bは実在しない年齢(「42歳」等)を3/3回再現性ある形で捏造**しており不採用。llm-jp-4-8b-instructは精度は高いが長文で700秒超と遅すぎる。Sarashina2.2-3Bは捏造0件、短文は高品質
- [x] **プロンプト改善(v2)**: Sarashina2.2-3Bの長文(8,000字)でのカバー率が56%(必須項目の脱落)だった問題に対し、必須5項目(種類/関係者/日付/金額/状態変化)を明示し箇条書き形式に変更するプロンプトへ改善、再検証でカバー率94%まで改善(数値の捏造は0件のまま)。ただし行数指定(5〜7行)は3Bモデルでは守られず10〜14行になる副作用あり
- [x] **ライセンス確認**: `sbintuitions/sarashina2.2-3b-instruct-v0.1`(および基盤モデル`sarashina2.2-3b`)はMIT、LICENSE本文(著作権者SB Intuitions 2025)まで直接確認済み。ゲート制限なし、商用利用・複数クライアント環境への配置とも問題なし。**同ブランドのTTS版(`sarashina2.2-tts`)のみ別ライセンス(非商用)のため混同注意**。モデル自体の公開日は2025-02-26(既存モデル、2026年の新着ではない。直近更新2026-08-25)
- [x] **追加候補の確認(PLaMo 2 8B/ELYZA-Diffusion-Instruct-1.0-Dream-7B)**: いずれもSarashinaより明確に優位とは言えないと判断。PLaMo 2 8Bは「PLaMo Community License」(年商10億円以下は無償だが利用登録フォーム提出必須、再配布にライセンス波及制限)でMITより制約が強く、サイズも3Bの約2〜3倍でCPUではより遅い。ELYZA-Diffusion(拡散言語モデル、Apache 2.0)は日本語ベンチマーク数値がモデルカードに記載なく未検証、CPU実測情報もなし
- [x] **ROI判断・棚上げ(2026-09-21)**: 要約は低頻度・手動実行のみでGemini依存脱却の締切圧力がOCR(Vertex AI東京リージョン非公式・Gemini 2.5廃止)と異なり現状ない。PLaMo/ELYZA等の追加検証は、現時点で判明している条件(ライセンス制約・サイズ・未検証)だけでSarashinaより不利な要素を抱えており、深掘りしても「Sarashinaを超える」結論に届く見込みが低いと判断し、decision-maker合意のうえ調査を打ち切った
- [ ] **次の一手(トリガー付き)**: 以下のいずれかが発生した場合のみ再着手する。①Gemini 3.5 Flash自体の東京リージョン非公式利用に関するリスクが要約用途でも現実化した場合 ②要約機能をSLMへ実装移行する明示決定が出た場合。再着手時は、本節の暫定候補(Sarashina2.2-3B、プロンプトv2、下記テンプレート)を起点にしつつ、PLaMo 2 8B(登録手続き含む)・ELYZA-Diffusionの実機検証も改めて検討する。検証用リソース(Cloud Run services `slm-bench-*`、Artifact Registry `slm-bench`)は毎回検証後にdevから削除済み、恒久リソースは未作成
