...
- [ ] G. dev環境A/Bテスト実行（4項目の精度・トークン増分・grounding失敗率確認、D,E,Fに依存。タスクFのハーネスで一次スクリーニング済み=baseline/candidate両方4項目100%一致・N=5、本実行では対象文書数を拡大して再現性を確認する）— **decision-maker承認済み(2026-07-14 session123会話内、着手指示あり)**。session123はcontext残量のため未着手のままhandoff。次セッションは追加確認なしで着手可(即着手タスク)。対象文書数拡大の具体的な手段(既存フィクスチャ追加 or 実データ活用)は未確定のため、着手時に軽量プランを提示してから実装
