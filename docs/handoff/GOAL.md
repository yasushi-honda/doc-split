...
- [ ] I. cocoro本番バックフィル実行 — **保留中(decision-maker判断: 2026-07-15、Issue #660修正を先に行う方針)**。kanameoneで見つかった既存グループのドリフト(Issue #660)がcocoroでも再現するリスクがあるため、Jの修正・検証完了後に着手する
...
- [x] J2. dev実装+検証 — `documentAggregationEvents/{event.id}`冪等台帳を新設し、`onDocumentWrite`の集計反映部分を単一トランザクション化(`transaction.getAll()`によるread phase→`applyAggregationDeltas()`+台帳`create`のwrite phase)。CloudEvent構築の複雑さを避けるため本体ロジックを`processDocumentAggregationEvent()`として独立させ直接テスト可能にした。新規integration test(`updateDocumentGroupsIdempotencyIntegration.test.ts`)でAC1(同一event.id二重実行)〜AC5(キー正規化no-opのリグレッション)の5件を実装、特にAC2(A→B→A順序不同再配信、単一フィールド案では失敗する核心ケース)を含め全PASSを確認。既存契約テスト(`groupAggregationRebuildContractIntegration.test.ts`)も新シグネチャで全PASS。dev環境のFirestore TTLポリシー(`documentAggregationEvents.expireAt`)設定・ACTIVE化も確認済み。全体検証: build/lint(既存warningのみ)/unit 1784件/rules 77件/integration 111件(新規5件含む)全PASS
- [ ] J3. kanameone/cocoroへのデプロイ+検証 — J2完了後、両環境へデプロイし実運用で新規書込みが正しく冪等に処理されることを確認
- [ ] J4. 過去ドリフトの是正判断 — J3完了後、`rebuildAllGroupAggregations()`による全件再構築を実施するか、個別グループの補正で済ませるか、decision-makerと相談のうえ決定。Codex助言: 冪等性修正の全環境配備前に全件再構築を行うのは安全ではない(ライブトリガーとの競合・非一貫スナップショットのリスク)
