...
<!-- session114: decision-makerから「当初のkanameコスト問題を基準に進捗はどうか」「翌月請求を待たずに今できることはないか」と問われ、read-only `check-gemini-cost-stats`(stats/gemini/daily)をkanameone/cocoro双方でGitHub Actions経由(read-only)で実行。分析は2回訂正が必要だった: ①初回「63倍」は旧pricing定数+旧B1未反映データとの誤比較 ②再検証の「9〜13倍」も、手動サンプル(session95、1ページあたりトークン数)と自動集計(1リクエストあたり平均)という異なる測定単位を混在させた誤り。21日分の同一システム内データで正しい単価(2.5系$0.3/$2.5)を両側に適用して再計算した結果、実効値上がり倍率は**kanameone約6.7倍・cocoro約5.0倍**で確定(#548試算の前提5.53倍とほぼ整合)。ボリュームも6月比で異常な落ち込みはなく通常の変動範囲内(前回「6月実測406ページ/日比13%」との比較は測定単位不一致による誤り)。詳細はLATEST.md session114サマリ参照 -->
...
- **session114実測（read-only `check-gemini-cost-stats`、kanameone/cocoro、21日分をGitHub Actions経由で取得）**: 移行前後を同一の測定システム内・正しい2.5単価（$0.3/$2.5）で再計算した結果、実効値上がり倍率は**kanameone約6.7倍・cocoro約5.0倍**（#548試算の前提5.53倍とほぼ整合）。これ以前に2回、より高い倍率（session112記載の「9倍」、session114初回の「63倍」「9〜13倍」）を提示していたが、いずれも「移行前データが旧・過小pricing定数で記録されていた」「手動サンプルと自動集計という異なる測定単位を混在させた」という測定手法上の誤りに起因しており、本項目の数値が正しい
- **ボリュームも6月比で異常な落ち込みはない**: 21日分のリクエスト数を見ると、kanameoneは移行前18日間で28〜391件/日と元々変動が大きく、移行後(39〜53件/日)は直近の移行前日(07-04/05: 46/31件)と同水準。cocoroも同様に通常の変動範囲内。以前提示した「6月実測406ページ/日比13%まで落ち込み」という比較は、別の手動サンプル測定（ページ単位、OCR再利用スキップ分を含む可能性）とこの自動集計（リクエスト単位）を単純比較した誤りだった
- **結論**: #548試算（全対策後 約¥23,000/月）は、これまでの2回の訂正で述べてきたほど楽観的すぎるものではなく、概ね妥当な水準と判断できる。ただし依然として3.5移行前(¥12,714/月)を上回る水準であることに変わりはない
...
- #548試算（全対策後¥23,000/月）の翌月請求での最終確認（session114のread-only再検証で概ね妥当な水準と判断済みだが、実請求での裏取りは未実施）
- A/Bテスト事前実測（3.7〜4倍）と本番実測（kanameone約6.7倍・cocoro約5.0倍）の残差原因調査（乖離は縮小したが依然存在、未着手・read-only範囲で可能）
