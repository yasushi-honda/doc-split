...
## 【調査完了・棚上げ・2026-09-21】要約生成(regenerateSummary)のGemini依存脱却: 自前ホスティングSLM調査 — Sarashina2.2-3B(Q8_0量子化)を暫定候補として記録、実装は着手せず

ADR-0025はPass1(OCR)のみ対象でPass2/要約は明示的にスコープ外(手動トリガー・低頻度のため)。decision-makerの意向で「要約もいずれはPaddleOCRと同様に自前ホスティングSLM(Cloud Run、CPUのみ、asia-northeast1)へモデルルーティングしたい」という将来検討として、2026-09-21に候補調査・実機検証を実施した。**今回は要約をGeminiに残す決定は変えていない**(同日「要約は今回は残す方針」とdecision-maker確認済み)。実装(summaryPromptBuilder.ts等の変更)は一切行っていない。
...
- [ ] **次の一手(トリガー付き)**: 以下のいずれかが発生した場合のみ再着手する。①Gemini 3.5 Flash自体の東京リージョン非公式利用に関するリスクが要約用途でも現実化した場合 ②要約機能をSLMへ実装移行する明示決定が出た場合。再着手時は、本節の暫定候補(**Sarashina2.2-3B・Q8_0量子化**、プロンプトv2、下記テンプレート)を起点にしつつ、①固有名詞捏造テスト(D9/D10相当、情報欠落パターン)を実装前の必須ゲートとして継続する ②PLaMo 2 8B(登録手続き含む)・ELYZA-Diffusionの実機検証(同じ捏造テストを含む)も改めて検討する ③実装時は本番コードの変更(`summaryPromptBuilder.ts`・feature flag・Cloud Runサービス新設等)を伴うためCLAUDE.md CRITICAL(5ファイル以上/新機能/アーキテクチャ判断)に該当し、plan modeでのフル計画とdecision-maker承認が必須。検証用リソース(Cloud Run services `slm-bench-*`、Artifact Registry `slm-bench`)は毎回検証後にdevから削除済み、恒久リソースは未作成
