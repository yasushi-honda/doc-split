...
- [x] **PR #967誤記訂正(2026-09-19、fable-review+自己検証)**: マージ後にPR6計画準備の調査で、PR #967に含めた「Gmail添付ポーリングが`processOCROnCreate.ts`経由で並行起動する」というバーストリスクの根拠が誤りと判明(`processOCROnCreate`はADR-0010で廃止済み・未デプロイのデッドコード、`gcloud functions list`で確認)。当初「バーストは起こらない」と結論づけかけたが、これも`/fable-review`による独立検証で**過剰訂正**と判明: 本番の実際のトリガー`processOCR`(Cloud Scheduler)は実機`maxInstanceCount=1`だが`maxInstanceRequestConcurrency=80`(既定値)であり、**1サイクルがポーリング間隔60秒を超えると次tickが並行実行され複数文書へのOCRリクエストが同時in-flightになりうる**(ADR-0023が2026-08-02にkanameoneで既に観測済みと記録)。現状(Gemini運用、1サイクル19〜33秒)は問題顕在化していないが、PaddleOCRは1件あたり遅くcold start外れ値もあるため**PR6後にこの60秒境界を超える確率が上がる**。訂正内容を反映(README.md/Issue #966/deploy-paddle-ocr.ymlコメント)。min-instances=1の評価も上方修正(現状トラフィックは完全直列のため実質全リクエストのcold startを解消)
