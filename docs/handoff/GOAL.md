...
## 【進行中・2026-09-12開始】ADR-0025 PaddleOCR: dev/kanameone/cocoro全3環境でインフラ・L1配線・実文書canary完了、Stage3負荷試験ハーネス実装・Phase B実測実行とも完了（2026-09-18）、**必須ゲート(71ページ)PASS、Pass1切替(PR6)のNo-Go要因なし。1ページcoldゲートFAILはmin-instances=1で部分的に緩和(PR #967)、Gmail添付バースト到着時の残課題はIssue #966で継続管理**（現在のミッションとは別件・並行トラック）
...
- [x] **1ページcold start緩和策の追加スパイク検証+恒久採用(2026-09-19、PR #967)**: dev環境で`min-instances=1`に変更し、孤立した単発リクエスト(3秒間隔×3回)で外れ値解消(6.2〜6.4秒)を確認。一方、意図的な3件同時バーストでは外れ値(34.5秒)が継続。**fable-reviewの指摘で判明した重要な訂正**: 当初「単一ページ文書の同時到達は稀」と判断したが誤りで、`checkGmailAttachments.ts`(`maxResults=50`)+`processOCROnCreate.ts`(`onDocumentCreated`に`concurrency`/`maxInstances`指定なし)によりGmail 1回のポーリングで複数文書がほぼ同時にOCRへ到達するバーストは実運用で起こりうる。`min-instances=1`はバーストの1件目のみを救う部分的緩和策と位置づけを修正のうえ、decision-maker承認により`deploy-paddle-ocr.yml`で恒久採用(`--min-instances=0`→`1`)。2026-09-19時点でdevのみ反映済み、kanameone/cocoroは次回dispatch時に反映(現状revision`00001`のままminScale=0を実機確認済み)。詳細はIssue #966・`services/paddle-ocr/README.md`「PR4c実測値」節参照
- [ ] **次の一手**: PR #967をレビュー(fable-review完了、High 3件/Medium 5件反映中)・マージのうえ、PR6(Pass1切替の実装)に着手する。本番切替という影響範囲の大きいアーキテクチャ判断のため、CLAUDE.md CRITICAL(5ファイル以上/新機能/アーキテクチャ判断)に該当し、**plan modeでのフル計画が必要**(親計画`~/.claude/plans/shiny-knitting-flamingo.md`のPR6節を起点に、L2 Firestoreフラグ・allowlist段階導入等の既存ロールアウト設計と整合させる。Gmail添付バースト到着パターンをPR6の負荷設計に織り込むこと)
