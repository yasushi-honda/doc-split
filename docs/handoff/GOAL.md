...
- [ ] Phase C（クライアント自己完結、外部依存）: kanameone/cocoro各管理者によるGoogle Drive OAuth接続・フォルダ選択・テンプレート保存。executor代行不可、着手待ち
- [ ] Phase D/E（要再設計）: feature flag ON・backfill本実行。Codexが指摘したHigh 5件（flag ON直後は他ユーザーの通常確認操作も全てDrive書込みトリガー対象になり「1件だけのコントロールテスト」が成立しない/backfillスクリプトに`--limit`等のcanary機構がない/「flag OFF」はロールバックにならない/通常操作とbackfillの競合/実行SA検証の甘さ）を踏まえ、次回セッションでplan modeにより再設計してから着手
...
- [ ] 【様子見・2026-07-22 handoff §4.6同根再発スキャンで新規発見】exchangeDriveAuthCodeCore、`setSecret`成功後の最終Firestore `set()`が失敗した場合、Secret Manager側は新refresh_tokenが`latest`に昇格済みなのに`settings/drive`のauthMode/connectedEmailは更新されない、という3a6409aで修正したsplit-brainと同根クラスの問題がFirestore書込み失敗という経路で再発しうる（functions/src/drive/exchangeDriveAuthCode.ts:107-115）。Secret ManagerとFirestoreは別システムのため真のアトミック性は構造的に担保できず、リトライ/補償ロジック追加は新規の設計判断を要する。Firestore書込みは高信頼性のため実害の蓋然性は低いと推定されるが、未検証・未対応
...
**対象タスク**: 現在のミッション「kanameone・cocoroへのGoogle Drive連携Phase1本番展開」の Phase C/D/E。

**直前の状態**: Phase A（Codexセカンドオピニオン）・Phase B（kanameone/cocoro双方のインフラ準備）まで完了・検証済み（詳細は上記「現在のミッション」節）。両環境とも「クライアントが設定画面でGoogle Drive連携ボタンを押せる状態」に到達。`settings/features.driveExport`は両環境ともflag OFF（未設定）のまま維持しており、現状は無害。

**次の一手**: 2つの独立した継続タスクがある。
1. Phase C（外部依存、executor代行不可）: kanameone/cocoro各管理者にGoogle Drive連携の操作手順（Drive連携ボタン押下→OAuth同意→フォルダ選択→テンプレート保存）を案内し、実施を待つ。
2. Phase D/E（要再設計、次回セッションでplan mode着手可能）: Codex指摘（flag ON直後の全ユーザー巻き込みリスク／backfillのcanary機構欠如／flag OFFはロールバックにならない等）を踏まえ、`scripts/backfill-drive-export.ts`への`--limit`/`--expected-count`追加、canary→全量の2段階実行設計、flag ON運用ルール（業務時間外実施 or allowlist機構）を設計してから着手する。

**検証コマンド**（再開時に現状確認）:
```bash
# 両環境のPhase B状態確認（flag OFF維持・関数ACTIVE確認）
gcloud functions list --project=docsplit-kanameone --account=hy.unimail.11@gmail.com --format="table(name,state)" | grep -i drive
gcloud functions list --project=docsplit-cocoro --account=hy.unimail.11@gmail.com --format="table(name,state)" | grep -i drive
# Phase C進捗確認（クライアントが接続済みか）: Firebase Console で settings/drive.authMode を確認
```
