...
<!-- session115: Issue #539完了。`/impl-plan`軽量→実装→`/safe-refactor`→`/code-review high`(8件検出、1件修正+3件Issue化)→PR #623→`/codex review`(P1指摘1件検出、修正自体の新規バグを追加修正)→CI PASS→マージ→Issue #539自動クローズ。マージ後最終状態を再度`/codex review`し指摘0件で健全性確認。次セッション起点をIssue #540に更新 -->
<!-- session119: catchup「次にやるとよいオススメ」から#620/#622/#626の3件をユーザー選択、dev実装→prod展開まで一気通貫で完遂。各PRは`/safe-refactor`→`/code-review low`(全件findings 0)、#626は3ファイル大規模のため追加で`/codex review-diff --base main`(findings 0)も実施。マージは番号単位認可(AskUserQuestionボタン)。PR #634(#620)/PR #635(#622)/PR #636(#626)、squash mergeでmain反映。dev実機確認: seed-dev-data --force-pendingでPDF2件(11ページ)を実OCR処理させ`documentsProcessed:2,errors:0,superseded:0`を確認(#626の新チェックロジックが11回とも正常通過)。GitHub Actions「Deploy Cloud Functions」でkanameone/cocoro両環境にデプロイ、`gcloud functions describe`の`updateTime`でデプロイ反映を確認。3件ともIssue自動クローズ済み -->
<!-- session120: catchup「次にやるとよいオススメ」からdecision-maker承認で3件着手。①前セッション未コミットのGOAL.md記録漏れをfeatureブランチ経由でPR化(main直push回避)。②監視事項「A/Bテスト事前実測(3.7〜4倍)と本番実測クリーン版(5.07倍)の残差原因調査」(read-only)を実施し解明: GitHub Actions実行ログ(run 28941729429、2026-07-08T12:19)を遡り、confirmed-replay方式(kanameone実データN=60、PR#592-594)のコスト比較結果「5.53倍」の生ログを発掘。この5.53倍はsession107(PR#596)で一度確定していた実測値だが、以降のセッションで出典を見失い「理論上の前提」として扱われ、比較対象として引用していたPR#559のA/Bテスト(n=11、dev合成fixtureの清潔な2ファイルのみ)との乖離が「未解明の残差」として記録され続けていた。5.53倍(confirmed-replay、実データ)と5.07倍(本番実測クリーン版)は9%差に収まっており、残差はconfirmed-replayのサンプル母集団バイアス(人間確定=過去に訂正が必要だった難しい文書のみ、N=60)で説明可能な範囲。結論: 「未解明の残差」は測定誤差ではなく、比較対象の選択ミス(小サンプル合成fixtureとの比較を続けていたこと)が原因だった。詳細は下記「重要な注記」に反映 -->
...
- **#547 Phase E が完了し、egress実削減効果が発生する**（証明: 本体 `documents/{docId}` から `ocrResult`/`pageResults` が `FieldValue.delete()` で削除され、一覧クエリの転送量が実測で減少）。**達成済み、2026-07-10 session113**: cocoro（1,055件、run [29102788032](https://github.com/yasushi-honda/doc-split/actions/runs/29102788032)で`--verify` PASS）・kanameone（9,513件、run [29130164136](https://github.com/yasushi-honda/doc-split/actions/runs/29130164136)で`--verify` PASS）両環境とも親doc残存0件・detail/main不在0件・エラー0件を確認。egress実削減自体は翌月請求で実測確認が必要（本項目は「削除実行が完了したこと」の証明であり「請求額が下がったこと」の証明ではない点に注意、下記「重要な注記」参照）。**注: Issue #547自体は2026-07-07 Phase B完遂時点で既にclose済み**（ADR-0018記載の意図的運用: 「Phase C以降は別途decision-maker起点指示+`/impl-plan`で着手」）。Phase C/D/Eの進捗はIssue再オープンではなく本GOAL.md + ADR-0018で追跡する
  - **是正: 2026-07-12 session117**: session113の「完遂」はPR-E1(`b1e8297`、dual-write停止コード)が本番Cloud Functionsに実デプロイ済みという誤った前提に基づいていた。実際にはmainマージのみでkanameone/cocoro双方に一度もデプロイされておらず、削除実行後も旧コードが再処理ドキュメントへ`ocrResult`/`pageResults`を書き戻し続けていた（read-only `--verify`でkanameone17件・cocoro5件の再発生を実測、detail/main不在は0件でデータ損失なし）。Codexセカンドオピニオンで是正計画を検証の上、両環境に`b1e8297`+#539+#540+#625をデプロイし、cleanup再実行→両環境最終`--verify` PASS（残存0件）で是正完了。詳細はADR-0018「是正: 2026-07-12 (session117)」参照。**egress実削減が実際に有効化されたのは今回の是正完了時点（2026-07-12）であり、session113時点ではなかった**
...
- **実効値上がり倍率は最終的に4手法が収束**: ①単価差のみの理論値3.88倍 ②A/Bテスト事前実測(PR#559、dev合成fixture n=11)3.7〜4.0倍 ③confirmed-replay実測(PR#592-594、kanameone実データN=60、2026-07-08)5.53倍 ④本番実測・交絡排除版(kanameone、OCR単体・モデル確定日のみ)5.07倍。Codexセカンドオピニオンの指摘（1日サンプルでの確定は統計的に不可、source mix交絡=移行前は自動要約混入・移行後はOCR単体という非対称性）を踏まえ、交絡を排除して再計算した結果がこの5.07倍。この過程で以前提示していた「63倍」「9〜13倍」「6.7倍/5.0倍」は測定単位の不一致や交絡未排除によるものと判明し、順次訂正済み。**session120で判明**: ③confirmed-replay(5.53倍)が実データベースの測定として④(5.07倍)と最も近く(差9%)、②(3.7〜4.0倍、清潔な小サンプル合成fixture)より妥当な比較対象。「A/Bテスト事前実測との乖離」として監視していた残差は②を誤って比較対象に選んでいたことが原因で、③の存在を踏まえれば実質解消（詳細下記）
...
### 🎯 次セッションの起点（decision-maker予定済み）
- ✅ **Issue #539**（splitPdf二重実行race）完了（2026-07-11 session115）。`/impl-plan`軽量モード承認→実装（早期status拒否+lastUpdateTime precondition排他制御、rotatePdfPagesと共通ヘルパー化）→`/safe-refactor`（DRY違反修正）→`/code-review high`（8 finders→1-vote verify、8件検出・全件CONFIRMED、1件〔retry時stale startUpdateTimeによるfalse-positive abort〕を本PRで修正、残り7件は#620/#621/#622へ分離）→PR #623作成→`/codex review`（P1指摘1件検出: 修正自体がstatus/fileUrl変化を見ずstale dataでsplit完了させる新規バグを検出→追加修正）→CI全PASS→マージ承認→squash merge（`054cf7a`）→Issue #539自動クローズ。マージ後の最終状態に対し再度`/codex review`実施、指摘0件で健全性確認済み
- ✅ **Issue #540**（processOCR古いスナップショット上書き）完了（2026-07-12 session116）。`/impl-plan`フルモード承認 → `/codex plan`（MCP high）でセカンドオピニオン取得、5件のHigh指摘を全て計画に反映（H3〔同一runId×入力drift時の即pending化〕のみ複雑性とのトレードオフで見送り）→実装（`ocrRunGuard.ts`新設: `evaluateOcrRunOwnership`+`OcrRunSupersededError`、`tryStartProcessing`のclaim-time snapshot化、所有権検証、Storageパスのrun毎分離）→`/safe-refactor`（修正不要）→`/code-review high`（8 finders→1-vote verify、8件検出3件CONFIRMED・1件〔ドキュメント削除とsupersededの誤混同によるerrors/記録消失〕を本PRで修正、2件は別Issue化〔#625 Storage孤児化P1・#626 Gemini浪費P2〕）→`/codex plan`で対応方針セカンドオピニオン確認→PR #627作成→hookが大規模PR判定し`/review-pr`（5エージェント）+`/codex review-diff`を実施、pr-test-analyzer/silent-failure-hunterが独立に発見した「handleProcessingErrorが所有権と無関係な本物のエラーもsafeLogErrorごと抑制する」設計欠陥を追加修正、superseded配線の純粋関数化+テスト追加→`/codex review-diff --base main`で新規指摘0件（既知のStorage孤児化のみ再確認）→マージ承認（AskUserQuestionボタン）→squash merge（`4a4b740`）→Issue #540自動クローズ。テスト: functions単体1708 passing + integration92 passing
- ✅ **Issue #625**（OCR結果Storageの孤児オブジェクト無制限蓄積）完了（2026-07-12 session117）。`/impl-plan`軽量モード承認→実装→Codexセカンドオピニオン（plan、MCP high）で当初設計の前提破綻（FE側`getReprocessClearFields()`が`ocrResultUrl`を事前削除するため機能しない）を検出、Storage prefix listingベースに設計変更→`/safe-refactor`（DRY指摘1件は見送り判断）→`/code-review low`でレースコンディション検出・修正→PR #629作成、CodeRabbit指摘（削除ループ内再検証不足）に対応→マージ→Issue #625自動クローズ
  - **副次的発見**: デプロイ状況調査中に、mainマージ済みの4コミット（#611 Phase E dual-write停止/#539/#540/#625）がkanameone/cocoro双方に一度もデプロイされていないことをground truth（`gcloud functions list`実測+handoff記録の裏付け）で確認。さらに調査したところ、2026-07-10のPhase E本番データ削除実行（`--mark-preflight`ゲートは人間申告のみで実デプロイ状態を検証しない仕組みだった）が、実際にはdual-write停止コード未デプロイのまま実施されており、削除後に再処理されたドキュメント（kanameone17件・cocoro5件、detail/main不在は0件でデータ損失なし）で値が本体へ書き戻されていたことをread-only `--verify`で実測確認
  - **是正実施**: Codexセカンドオピニオンで是正計画（デプロイ→drain待機→verify比較→dry-run→execute→最終verify、cocoro先行→kanameone後続）を検証の上、各ステップ番号単位認可を得て実施。cocoro: デプロイ成功→verify横ばい確認→execute（削除成功5/エラー0）→最終verify PASS。kanameone: デプロイ成功→verify実行時に17→19件（drain待機中の旧リビジョン処理完走による想定内の一時増加、再verifyで横ばい確認）→execute（削除成功19/エラー0）→最終verify PASS。両環境とも`fix-stuck-documents --include-errors --dry-run`でエラー・スタック文書0件を確認。ADR-0018に是正記録を追記（session113の「完遂」記述を訂正）。**egress実削減が実際に有効化されたのは本日（2026-07-12）**
  - **追加の見落とし発見・是正**: `b1e8297`(#611)はFunctionsだけでなくfrontend(`useDocuments.ts`/`DocumentDetailModal.tsx`/`PdfSplitModal.tsx`、`detail/main`取得中・失敗時のPDF分割ブロックゲート等)も変更していたが、Hostingは両環境ともPhase D時点(2026-07-09)が最後で未反映だった(`deploy-hosting.yml`実行履歴で確認、kanameone専用workflow・cocoroはローカルCLI手順)。Functions側dual-write停止が先に本番展開されたことで、新規docの`pageResults`が`detail/main`のみになった状態と、旧FE(ゲートなし)の組合せにより「`detail/main`取得未完了のままPDF分割を進めるとpageResults欠落でdocumentType/customerNameが黙って未判定化する」というサイレント品質劣化リスクが顕在化しうる状態だった。kanameoneは`deploy-hosting.yml`、cocoroはローカルFirebase CLI(`hy.unimail.11@gmail.com`、`scripts/deploy-to-project.sh cocoro`)でHostingデプロイを実施し解消(両環境デプロイ成功確認済み)
  - 守り（検出）で新規起票したIssue: #625（Storage孤児化、P1、reprocess毎に旧ocrResultUrlオブジェクトが無制限蓄積、**完了 2026-07-12 session117**）・#626（Gemini浪費、P2、所有権チェックが最終tx時のみでsupersededされたrunがAPIコストを消費し切ってから破棄、**完了 2026-07-12 session119、PR #636**。PDFページループ内で各ページGemini呼出し前に`checkOcrRunStillOwned`早期チェックを追加、dev実機でOCR処理正常完了確認済み、kanameone/cocoro両環境デプロイ済み）

### 守り（検出）で新規起票したIssue（#539のcode-review副産物、全件完了）
- #620: NOT_FOUND(親doc削除)がsplitPdf/rotatePdfPagesの`isFirestorePreconditionFailure`で「二重split」と誤診断されるメッセージ精度問題。**完了 2026-07-12 session119、PR #634**（`isFirestoreNotFound`ヘルパー追加、NOT_FOUND時は「parent document was deleted during processing」の正確なメッセージに分岐）
- #621: splitPdf新エラーコード(already-exists/aborted)がFE未対応（CLAUDE.md API境界確認ルール抵触、生エラー表示+キャッシュ未invalidate）。**完了 2026-07-12 session118、PR #632**
- #622: `splitPdfConcurrencyGuardContract.test.ts`のgrep contractがsplitPdf固有の配線を実質保証できていない(vacuous test risk)。**完了 2026-07-12 session119、PR #635**（`extractSplitPdfFunctionBody()`でsplitPdf本体スコープに限定、`isFirestorePreconditionFailure`をexport化しintegration testの手動複製を解消）
...
- ~~A/Bテスト事前実測（3.7〜4倍）と本番実測クリーン版（kanameone5.07倍）の残差原因調査~~ → **session120で解明・解消**: 比較対象がPR#559(dev合成fixture n=11)だったのが誤り。confirmed-replay実測(PR#592-594、kanameone実データN=60)の5.53倍が本来の比較対象で、本番実測5.07倍との差は9%（サンプル母集団バイアスで説明可能な範囲）。詳細は上記「重要な注記」参照
