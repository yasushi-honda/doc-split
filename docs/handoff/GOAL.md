...
- [x] Firestoreトリガー実装（functions/src/drive/driveExportTrigger.ts + test。二重エンキュー防止をFirestoreトランザクションでアトミック化（/code-review low指摘対応）、commit予定）
...
## 🔄 中断点（in-flight）
なし（Task7は`/code-review low`指摘（二重エンキューガードのTOCTOUレース）をFirestoreトランザクションで修正し、回帰テスト追加（並行呼び出しでexportDocumentが1回のみ実行されることを検証）、全テストグリーン確認後にcommit予定。Task8以降は未着手のため部分着手状態のタスクなし）
