...
- [x] **`/handoff`同根再発スキャン(§4.6)でIssue #954起票**: Issue #952完了直後のhandoff実施時、「過去7日以内に同症状の修正PRが複数ある場合は同根候補を調査する」プロトコルに従い`grep`でコードベース内の`runTransaction`使用箇所を横断確認したところ、`functions/src/drive/driveFolderClaim.ts`(Issue #871 claimプロトコル)に同型の無保護`runTransaction`が11箇所存在すると判明。1箇所(`commitResolvedWithRetry`)は`withBackoffRetry`+型付きエラーで一部緩和済みだが、残り10箇所は無保護。decision-maker明示指示によりIssue #954として起票（監査依頼のみ、修正は未着手）
