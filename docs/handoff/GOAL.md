...
- 🔄 **Issue #540**（processOCR古いスナップショット上書き）実装中（2026-07-12 session116）。`/impl-plan`フルモード承認 → `/codex plan`（MCP high）でセカンドオピニオン取得、5件のHigh指摘（claim-time snapshot不在/handleProcessingError所有権未検証/Storageパス衝突/processOCROnCreate.ts旧シグネチャ/コスト計測漏れ）を検証の上すべて計画に反映（H3〔同一runId×入力drift時の即pending化〕のみ複雑性とのトレードオフで見送り、rescueStuckProcessingDocsの既存10分救済に委ねる安全側の選択）→実装完了: `ocrRunGuard.ts`新設（`evaluateOcrRunOwnership`純粋関数+`OcrRunSupersededError`）、`tryStartProcessing`がclaim-time snapshotと`ocrRunId`を返す設計に変更、`processDocument`/`handleProcessingError`/`saveOcrResult`(Storageパスをrun毎に分離)に所有権検証を追加、`processOCR.ts`/`processOCROnCreate.ts`が新シグネチャに追従。テスト: `ocrRunGuard.test.ts`(単体15件)+`ocrRunGuardIntegration.test.ts`(Firestore Emulator統合6件、新規)+既存`rescueErroredIntegration.test.ts`の`handleProcessingError`呼出し6箇所を新シグネチャに更新。**全テストPASS確認済み**: functions単体1705 passing、integration全91 passing（新規+既存回帰とも）、`npm run build`エラー0、lint 0 errors（26 warningsは全て変更前から存在する無関係ファイルのもの、processOCR.tsの`no-await-in-loop`警告5件は変更前後で同一パターンと確認済み）。
  - 🔄 中断点: 実装・テストは完了。**次: `/safe-refactor` → `/code-review`（quality gate、8ファイル変更のためフル発動条件に該当）→ PRを作成しCloses #540 記載 → マージ承認は番号単位で仰ぐ**
