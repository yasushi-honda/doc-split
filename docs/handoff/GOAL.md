...
- [ ] J3. kanameone/cocoroへのデプロイ+検証 — J2完了後、両環境へデプロイし実運用で新規書込みが正しく冪等に処理されることを確認。TTLポリシー(`gcloud firestore fields ttls update expireAt --collection-group=documentAggregationEvents --enable-ttl`)の各環境での設定も必須(codex review指摘、devのみ実施済み)
