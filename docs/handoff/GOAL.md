...
- [ ] リトライ成功時、成功メッセージがuseRetryDriveExportのクエリ無効化で即座にリストから消え去りユーザーが読めない（CONFIRMED。frontend/src/pages/ErrorsPage.tsx:591。行コンポーネントのローカルstateに保持しているため、成功→driveExportStatus変化→一覧再取得→該当行アンマウント、という経路で表示前後即消失。sonner toastへの置換を検討）
- [ ] handleRetryのcatchブロックがキャッシュ無効化を一切行わずリトライ失敗時に一覧が古い状態のまま固着する（frontend/src/pages/ErrorsPage.tsx:605。定期スイープや別管理者との競合でfailed-preconditionを受けた場合、実際には解決/処理中かもしれない古い表示が「更新」ボタン手動押下まで残る）
- [ ] failed-precondition素通し分岐が既存exchangeGmailAuthCode/exchangeDriveAuthCodeの英語技術メッセージ「No refresh token returned...」までUIにそのまま出す回帰（CONFIRMED。frontend/src/lib/callFunction.ts:85。今回の新機能向けの分岐追加が無関係な既存Gmail再連携フローに副作用。BE側でエラーコード別に「表示可否」ポリシーを明示するか、FE側で対象コードをDrive関連のみに限定する等の対応を検討）
- [ ] documentCategoryセグメントにも空文字ガードが無く書類種別未判定documentがフォルダ作成エラー/空白を起こしうる（functions/src/drive/folderPath.ts:120。careManagerName空文字の既知課題と同型、customer側のみFuriganaMissingErrorで保護されている非対称の再確認）
- [ ] findOrCreateFolder()に異なるdocId間の排他制御が無く、同一フォルダパスに解決される2件が近接タイミングでverifyされると重複フォルダが作成されうる（functions/src/drive/findOrCreateFolder.ts:38。ADRはdocId単位の再入TOCTOUのみ既知リスクとして記載、フォルダレベルの異docId間競合は未文書化・未テスト）
- [ ] 新設「リトライの確認」Dialogが実ブラウザで一度も開かれておらずCLAUDE.md「UIコンポーネント変更時の確認（#193教訓）」未達（CONFIRMED。frontend/src/pages/ErrorsPage.tsx:714。リトライボタン自体の実行は本番Drive副作用リスクのためdecision-maker判断でスキップ済みだが、Dialogを開くところまでは実ブラウザ確認せず終えた）
- [ ] [PLAUSIBLE、現状実害なし] useRetryDriveExportのonSuccessが`['document', docId]`クエリを無効化しておらず将来的にDrive状態を読むuseDocument(docId)呼び出しが古いキャッシュを表示しうる（frontend/src/hooks/useDriveExportErrors.ts:86。現時点でuseDocument(docId)の唯一の呼び出し元DocumentDetailModal.tsxはDrive系フィールドを読んでいないため実害なし）

## 🔄 中断点（in-flight）
なし。FEエラー一覧+リトライUI実装はcommit a352f43（実装）+6571041（GOAL.md更新）で反映済み・origin未push（実機確認・tsc/lint/frontend全459件PASS確認済み後にコミット）。上記の`/code-review xhigh`（2026-07-22）で新規CONFIRMED 4件+新規発見3件を検出、GOAL.mdタスク化済み・未着手。

dev環境のFirestore `settings/drive`は実際に`hy.unimail.11@gmail.com`と連携済み・`rootFolderName:'事務'`・`template`にかなめ式5階層が設定された状態のまま残っている（後続のE2E疎通確認タスクでそのまま利用可）。次は①E2E疎通確認 ②今回+前回2回分の`/code-review`指摘（新規7件+xhigh新規7件+high残り7件+PLAUSIBLE2件、計23件）のtriage、のいずれか。
