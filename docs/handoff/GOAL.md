...
- [ ] **次の一手**: PR6(Pass1切替のロールアウト計画)に着手する。本番切替という影響範囲の大きいアーキテクチャ判断のため、CLAUDE.md CRITICAL(5ファイル以上/新機能/アーキテクチャ判断)に該当し、**plan modeでのフル計画が必要**(親計画`~/.claude/plans/shiny-knitting-flamingo.md`のPR6節を起点に、L2 Firestoreフラグ・allowlist段階導入等の既存ロールアウト設計と整合させる)。**processOCRのtick重複リスク(上記訂正参照)への対応方針(`concurrency=1`明示設定の要否、サイクル所要時間の監視指標化)をPR6計画に含めること**
