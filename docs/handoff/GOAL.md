...
- [ ] J1. `/impl-plan`起票 — Codexが提案した修正方針(event.idベースの処理済みイベント記録+全affected group更新を単一Firestoreトランザクション化)を精査し、フル計画を作成。5ファイル以上・アーキテクチャ影響のため`/impl-plan`フルモード必須。本番Cloud Functionsのコアトリガー変更のため`/codex plan`セカンドオピニオンも実施すること
- [ ] J2. dev実装+検証 — 単体テスト(A/B自己書込みの結合テスト・同一event.id二重実行テスト・部分失敗リトライテスト)を含めて実装、Firebase Emulatorで検証
