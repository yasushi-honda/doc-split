...
| 環境 | 状態 |
|------|------|
| dev | デプロイ済み（02-06、e554641反映）※PWA追加は次回デプロイで反映 |
| kanameone | デプロイ済み（02-06、e554641反映）※同上 |

**要デプロイ**: PR #69（PWA対応）はmainにマージ済みだが未デプロイ。次回 `./scripts/deploy-to-project.sh <alias>` で反映される
...
1. **PWA対応のデプロイ**（dev/kanameoneへ `./scripts/deploy-to-project.sh` 実行）
