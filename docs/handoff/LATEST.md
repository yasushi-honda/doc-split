...
**更新日**: 2026-07-06 session101（#547 Phase B着手。/impl-planでADR-0018 Phase B実装計画を策定、Fable5との対等ピアレビュー2ラウンドで既存バグ(retryCount等7フィールドのwhitelist漏れ)を発見・emulatorで実証、5PR構成に確定。タスク0(splitInto実データ最大長調査、PR #568)完了。PR1(Firestoreルール改修+既存バグ修正、PR #569)完了・Codexレビュー3ラウンド(サーバー専有フィールド書換え禁止漏れ→null挿入脆弱性修正→収束)・dev実機確認済み。次はPR2(Functions dual-write改修)からcontext新鮮な状態で再開）
...
5PR構成の実装計画は`/impl-plan`+Fable5ピアレビューで確定済み（session101で番号単位認可+`/impl-plan`着手指示を充足、以降の各PR実装はこの計画の続行）。PR1(ルール改修)は完了済み。残り4PRは以下の順で着手可能、各PRのマージ時は引き続き番号単位認可が必要:

| # | 項目 | ROI | 想定工数 | 完了条件 | 関連ファイル |
|---|------|-----|---------|---------|------------|
| 1 | **PR2: Functions dual-write改修** | #547 Phase B本丸。PR1(ルール)完了済みで着手ブロッカーなし | 中〜大（5ファイル+Codexレビュー） | `ocrProcessor.ts`(tx内detail/main set+ocrExcerpt算出)/`pdfOperations.ts` splitPdf(batch内detail/main set、子doc上限249化)/`uploadPdf.ts`/`checkGmailAttachments.ts`/`import-historical-gmail.js`(全て同一tx/batch)を実装、テスト全PASS、Codexレビュー実施、PR作成・マージ | `functions/src/ocr/ocrProcessor.ts` / `functions/src/pdf/pdfOperations.ts` / `functions/src/upload/uploadPdf.ts` / `functions/src/gmail/checkGmailAttachments.ts` / `scripts/import-historical-gmail.js` |
| 2 | **PR3: 削除同期改修** | PR2と並列着手可（独立ファイル） | 中 | `deleteDocument.ts`(db.batch化)/`cleanup-duplicates.js`(BATCH_SIZE 500→250)/`cleanup-ambiguous-collision-docs.ts`(detail/main削除追加、lastUpdateTime preconditionは付けない)を実装 | `functions/src/documents/deleteDocument.ts` / `scripts/cleanup-duplicates.js` / `scripts/cleanup-ambiguous-collision-docs.ts` |
| 3 | **PR4a: FE writeBatch化3箇所** | PR1完了済み（ルール依存解消）で着手可能 | 中 | `useReprocessDocument()`/`useErrors.ts`の`requestReprocess()`/`DocumentsPage.tsx`の`handleBulkReprocess()`をwriteBatch化。**detail/main書込は投入しない**（ADR Phase B/D行に従いPR4bへ分離、親docのみ+ocrExcerptクリア）。250件チャンク化+途中失敗時のtoast表示 | `frontend/src/hooks/useDocuments.ts` / `frontend/src/hooks/useErrors.ts` / `frontend/src/pages/DocumentsPage.tsx` |
| 4 | **PR5: seed-dev-data.ts dual-write化** | PR2〜4完了後でも並列着手可、Phase C以降のdevリハーサル前提 | 小 | `buildProcessedDoc`/`buildPendingDoc`が生成するdocのdetail/mainへのdual-write追加 | `scripts/seed-dev-data.ts` |

各PR完了時は`/code-review`+Codexセカンドオピニオン（ADR-0018本文の必須要件）を実施すること。PR4b（detail/mainのFE書込、Phase D以降）は本計画のスコープ外、別Issueとして扱う。
...
> ⚠️ 即着手タスク4件はいずれもPR単位のマージに番号単位認可が必要（実装着手自体は5PR計画の続行として可能）。Fable 5は2026-07-07失効済の見込み。次セッション以降はSonnet 5 + Opus 4.8（計画時）を想定。

### 最終結論（session101末尾）

✅ **セッション終了可**

- OPEN PR: 0件（#568/#569すべてmerge済close） / active Issue: #547（Phase B着手済、タスク0+PR1完了・PR2〜5継続中）+ #526・#548（展開待ち）+ #539/#540/#503/#251/#238（トリガー待ち）
...
- 残留プロセス: なし
- 即着手タスク: 4件（PR2〜PR5、5PR構成計画の続行） / 条件待ち: 7件
- Issue Net変化: close 0件 / 起票 0件 = Net 0。#547 Phase B着手(タスク0+PR1完遂、既存バグ2種発見・修正)という実質進捗があるが、#547自体はPhase C以降も残るためopenのまま継続、Issue単位のNetには非反映
- §4.6 同根再発スキャン: 本セッションの`fix:`プレフィックスコミットは3件（すべてPR #569内、fixtureドリフト解消/サーバー専有フィールド書換え禁止/null挿入脆弱性修正）。いずれも同一PRのCodexレビューサイクルによる段階的精緻化であり、過去7日のhandoff archiveで同キーワード（retryCount/provenance/firestore.rules permission）のヒットなし、同根再発候補0件
- §4.7 対症療法判定: 該当なし。全修正はretry/fallback等の症状遮断ではなく、コード比較・emulator実証によるロジック欠陥の根本修正（Fable5指摘のwhitelist漏れはemulatorで再現確認、Codex指摘のnull挿入脆弱性はコードロジック検証の上で確定）
- 既知のblocker: なし。次セッションはPR2(Functions dual-write改修)からcontext新鮮な状態で着手可能
