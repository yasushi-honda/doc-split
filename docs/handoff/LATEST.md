...
**更新日**: 2026-07-11 session114（decision-makerの「そもそものkanameコスト問題を基準に今どうか」「翌月請求を待たず今できることはないか」という問いに対し、read-onlyのコスト実測`stats/gemini/daily`をkanameone/cocoro双方で確認。**単価の実効上昇は9〜13倍（#548試算5.53倍を上回る）だが絶対額はB1相殺で6月実績と同水準〜やや高め程度に留まる。月額を最終的に左右するのは流量（現状39〜53件/日 vs 6月406ページ/日）**。詳細は下記session114サマリ参照）
...
- GOAL.md「重要な注記」セクションをsession114時点の再検証結果で更新（詳細はGOAL.md参照）。
...
- **実測データ取得**（読み取り専用`check-gemini-cost-stats.js`をGitHub Actions経由でkanameone/cocoro向けに実行。**この実行はauto-mode権限classifierに一度ブロックされ、ユーザー承認を得てから結果確認**）: kanameoneで移行前(07-08)→移行後(07-10)のリクエスト単価が約9倍に上昇（試算の5.53倍よりさらに悪化、ただしサンプルは2日分と小さい）。
