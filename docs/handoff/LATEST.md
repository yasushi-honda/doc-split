...
### B. #220 関連: 監視基盤 dev/本番展開 — dev 完遂 (session6), 本番は session6 末尾で誤診、実際は即実行可能

**session6 で A1 (dev setup dispatch) 完遂**。dev 環境 5 metrics + 5 alert policies + 1 channel 稼働開始。本番 (kanameone/cocoro) は session6 中盤で「Owner 認証ブロッカー」と**誤診して持越し**を決めたが、session6 末尾で既存資産 (`switch-client.sh` + named config) の見落としが判明。**実際は `./scripts/switch-client.sh kanameone` で Owner コンテキストに即切替可能**。次セッションで switch-client.sh 前提の手順で実施すれば完遂できる（所要時間 各 ~15 分）。
...
1. **Follow-up B 残件 A2/A3 (~各 15 分、`switch-client.sh` で実行可能)**: kanameone/cocoro の監視基盤展開。**必須**: 各環境の named config（事前整備済）を `./scripts/switch-client.sh <env>` で切替してから実行（PR #247 で CLAUDE.md に必須プロトコルとして明記）
...
**重要な訂正** (session6 末尾): 当初「Owner 認証ブロッカー」と誤診して持越しを決めたが、実際は既存資産 (`switch-client.sh` + named config) の見落としが原因だった。正しい手順は以下の通りで、次セッションで着手可能。

### session6 で判明した正しい手順の前提
...
### session6 で実施済のクリーンアップ

- kanameone に roles なしで残骸として作成された SA (`docsplit-monitoring-admin`) は session6 末尾で削除済。次セッションは Step 3 から着手可能（SA 作成を含む手順をそのまま実行）
