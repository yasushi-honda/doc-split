# ハンドオフメモ アーカイブ (2026-06)

session79〜93（2026-05-16〜2026-07-03）の詳細記録。session94 で LATEST.md 圧縮のため本ファイルへ移動（フォーマット drift により wc -l ベースのアーカイブ閾値が session87 以降機能していなかった件の是正、詳細は session94 handoff 参照）。

---

**更新日**: 2026-07-03 session93 (**kaname 新規要望 B/C/D/E/F 受領 → 5 要望を精査・スコープ判断 (Codex セカンドオピニオン反映) → Issue 化 → dev seed 基盤 (#528) → B/D/F 実装・マージ・close 完了、E (#526, P1) は設計判断ゲート付きで次セッション持越し。Issue Net -1 (起票5/close4) だが 4/5 完了 + hook 誤検知 2 件をセッション内で発見・即修正**)。kaname から B(再処理ボタン配置拡張)/C(4/20 平出さん報告、既に PR #424 で対応済み)/D(ページ数表示)/E(手動分割→未登録問題)/F(担当CM別フォルダ構造変更) の 5 件を受領。契約外フィードバックのため kaname への確認往復はせず、Codex MCP セカンドオピニオン (plan mode) を得た上でプロダクト判断としてスコープ確定: **B** = グループビューへの再処理導線拡張（processed への行単位開放は除外、グループ見出し単位の一括処理も除外）、**F** = 「書類グループ」＝書類種別と解釈。triage 基準 #5 (ユーザー明示指示の複数タスク) 該当のため Issue #524/#525/#526/#527 起票 + 検証基盤 Issue #528 起票。**dev seed 基盤 (#528, PR #529)**: `scripts/seed-dev-data.ts` 新規 (マスター21 + processed/error 148 + pending 2、GHA `run-ops-script.yml` 経由が正規経路)。`/code-review medium` で 8 finding 検出 → 全件修正 (STORAGE_BUCKET 推測禁止違反 / totalPages と実PDF不整合 / pending 再投入による検証状態破壊 等)、GHA 実行で Firestore 171 docs + Storage 5 PDF 投入確認、実 OCR パイプライン E2E 実証 (混在FAX → OCR完了 → メタ自動抽出)。**Phase 1: B (#524, PR #530)**: `useReprocessDocument` フック新設 (詳細モーダル + グループビュー共通化)、詳細モーダルの再処理を error 限定から解放 (processed は「AIで再解析」表記)、確認ダイアログに消去項目明示、グループビュー行 (フラット+担当CM別サブグループ) に error 時のみ「再試行」。emulator E2E 7 件 + Playwright MCP 実機確認 (Firestore pending 化まで実証)。**Phase 2: D (#525, PR #531)**: 一覧テーブル (xl 以上) + グループビュー行にページ数表示、totalPages:0 は「-」フォールバック。PR #424 の 768〜1023px overflow 回帰なしを viewport 3 種 (768/1024/1280px) で実機検証 (768px の既存 15px overflow は stash 比較で変更前と同一値と実証、本 PR 起因でないことを確認)。**Phase 3: F (#527, PR #533)**: 担当CM別タブを 担当CM→利用者→書類種別→書類 の4階層に変更、`groupByDocType` 新設 (名前順、未判定は末尾)。デスクトップ+モバイル375px で4階層遷移を実機確認。**hook 更新 (decision-maker 指示)**: 従来無条件ブロックだった `ui-change-merge-check.sh` を「CI全PASS + `ui-verified` ラベル (確認証跡PR記録必須)」の条件付き許可に変更 (PR #532)、あわせてすり抜け穴2件 (PR番号誤認/番号なし素通り) 修正。**同根再発の即時発見・修正 (PR #534)**: handoff 作業中の実操作で、PR #532 で導入した false positive 対策自体に新たな盲点 (grep OR構文 `\|` がシェルのパイプ区切りと誤認) を発見、その場で修正・検証 (§4.6 同根再発スキャン相当を実質的に実施)。**残作業**: Phase 4 E (#526, P1) は分割→OCR再処理連携。`splitPdf` (Issue #432 P0 の震源) の挙動変更を伴うため、設計判断ゲート3点 (①手動入力メタの保持方針 ②OCR課金増の許容 ③再処理中の一覧表示) を提示してから着手する必要があり、次セッション最優先タスク。**Issue Net**: 起票5 (#524/#525/#526/#527/#528) - close4 (#524/#525/#527/#528) = **Net -1** (CLAUDE.md 基準では進捗ゼロ扱いだが、5件中4件を同一セッションで完了させた実質進捗であり、残る#526はP1・設計判断必須のため意図的に次フェーズへ温存)。

**更新日 (前)**: 2026-07-02〜03 session92 (**kaname 問い合わせ (「保存・確認済みにしても選択待ち/候補表示が残る、4/13 からチェックがつかない」) → 原因は Issue #492 Ambiguous 重複 docs と実証 → postponed 解除 (再開条件 1 充足) → 方針 A データ駆動版で Phase 1+2 全量整理 → Issue #492 close、Net -1**)。**調査 (read-only)**: 4/13 に kanameone hosting deploy なし (リリース履歴実測: 3/16 の次は 4/26) + 本番 bundle は #396 修正込み 5/18 ビルド + deploy 済み rules は repo と同一 → コード起因を棄却。session83 classify artifact (ローカル `.artifacts/session83/`) で manual-review 除外群の実体 = **20260413 8 グループ (同一親 `qwCIJLBk…`・同一ページ範囲・fingerprint 一致の真の重複 24 docs) + 20260509 2 グループ (別親 = Gmail 重複受信由来 5 docs)** と特定。auto classifier の本番 PII read ブロックを user 承認 + **field mask (フラグ + 未判定形式 fileName のみ、PII 非取得)** で回避し batchGet 実測 → **クライアント操作は正しく反映済み** (p1-4 の 1 doc を 5/26 編集・確認済み化、同名の残 2 docs が選択待ちのまま並び「直らない」ように見えていた) = 症状メカニズム実証。「4月13日」はデプロイ日でなく**書類の日付**。**Phase 1 (PR #520、6 files +1081)**: `scripts/cleanup-ambiguous-collision-docs.ts` + `scripts/lib/ambiguousCleanup.ts` (all-or-nothing preconditions、ユーザー操作済み doc 削除禁止) + plan JSON 固定 + run-ops-script.yml 登録 + backup artifact (90 日)。Codex review xhigh (P1: atomic batch + lastUpdateTime precondition / P2: fixture cleanup 404 限定) + /review-pr 5 並列 (HIGH: --execute の STORAGE_BUCKET fail-fast + 成功ログ正直化 / Important: 事後検証を winner 実 fileUrl 由来に / winner-in-losers defense-in-depth 等) 全反映、dev リハーサル 14 runs (violation reject 実証含む) → merge `9b5e6ed` → kanameone dry-run → 番号認可 → **execute run `28622728191`: 16 docs 削除、winner 8 現存、独立検証一致、トリガー連動 (search index / documentGroups) 確認**。**Phase 2 (PR #521、5 files +692/-133)**: user 判断「明白な重複 = 改修前システムの弊害、クライアント確認不要で修復」。Codex review が「20260509 は fileUrl 共有前提不成立」と指摘 → **本番実測で事実確認** (#432 復旧で各 doc が専用 path に移行済み、旧ロジックは fail-closed で全件 abort する状態) → **expectedParents + expectedFileUrls (実測 fileUrl pin) + 専有 object の storageGuard guarded 削除** (共有 object は不可侵維持) に拡張。/review-pr 3 並列 (F1: 不可逆ゾーンの postErrors flush 保証 / F2: 復旧手順出力 / F3: pre-flight 前倒し / T1-T5 defense-in-depth テスト) 全反映、テスト 43 cases + functions 全 1496 PASS、dev リハーサル (専有 object 2/2 削除 + 共有不可侵) → merge `6e78a17` → dry-run → 番号認可 → **execute run `28626359967`: 2 docs + 専有 object 2 件削除**。ユーザー確認済み `U4Lf5ZPN…` は安全装置に従い意図的残置 (p3 は確認済み 2 docs が現存、内容同一)。**Issue #492 close (completed) = Net -1**。kaname への返信は user が「解決済み」として伝達 (AI ドラフト提供済み)。**同根再発スキャン**: root cause (fileName collision 設計) は #432 で構造修正済み + 残骸データ全量除去完了 = 対症療法非該当 (根治)。意図的残置は p3 確認済み 2 docs のみ (Issue close コメントに文書化)。**環境ノート**: 本番 Firestore read は field mask で PII 除外すれば classifier 通過可 / Issue コメント・ラベル操作は別途ブロックされ得る (PR 経由で代替)。**追記 (23:01 UTC)**: user 質問「cocoro にも影響は?」を受け read-only audit-storage-mismatch を cocoro で再実行 (run `28626872745`) → **totalDocs=990 (5/17 の 23 件から成長) / fileName collisions 0 groups / orphans 0** = cocoro 影響なしを現時点データで再確定、マルチクライアント確認マトリクス完結 (dev=実データなし / cocoro=健全 / kanameone=本セッションで全量整理済)。残 open Issues = **#503 / #251 / #238 (P2、トリガー未充足 2026-07-02 実証済み)** の 3 件のみ。次セッション候補 = (a) P2 3 件は各 Issue 本文トリガー充足時のみ着手、(b) kaname から追加反応 (5/9 の確認済み 2 行の扱い等) があれば対応。

**更新日 (前)**: 2026-07-02 session91 (**ADR-0017 運用実績確認 (read-only 3 環境監査) → Status を Accepted に格上げ (PR #518) + 残 P2 Issues 3 件の着手トリガー未充足を実証確認、Net 0**)。session90 handoff 次セッション候補 (c)「ADR-0017 Accepted 格上げ判定 (運用 1-2 週間後)」の時間条件が 3 週間経過で充足 → catchup 即着手タスクとして運用実績確認を実施。**運用実績 (2026-06-12 deploy 以降)**: **kanameone 2026-06-16 に約 7.9h (06:15-14:08 UTC) の 429 ストーム (ログ 683 件) 発生 = 発端事象 (39 分) の約 12 倍規模**。主防御 (cumulative 3.5h horizon) を超過して 6 doc が error 化したが、backstop `rescueErroredDocuments` が 1h 間隔 scan で全件自動 rescue (いずれも rescueCount 1/3 初回復帰) → 全件完走 = **ADR Risks 欄「3.5h 以上枯渇 → rescue で完走を待つ」シナリオを実戦実証、手動介入ゼロ**。6/17 429 ×104 件は主防御のみで完全吸収 (error 化ゼロ)、7/1 ×2 件も吸収。6/12 03:47 の rescue 10 件は deploy 直後に 6/11 残留 error doc を自動回復したもの (設計通り)。cocoro は 429 ×2 件のみ (in-call retry 即吸収、rescue 発火なし)、dev は 429 ゼロ。**確認時点で 3 環境すべて status=error doc 数 0** (Firestore runAggregationQuery 実測)、rescue 上限 (3 回) 到達ゼロ。→ user 番号認可下で **PR #518** (docs-only 1 file +8/-1、ADR-0017 Status → Accepted + 格上げ根拠追記) squash merge `7a871e1`、deploy CI success。**残 P2 Issues 着手トリガー検証 (read-only)**: user「ctx 余裕あり、最適解は?」に対し 3 件の Issue 本文記載トリガーを実測検証 → **#503** (トリガー = sanitize drop の実運用発生): kanameone/cocoro 直近 30 日 `[loadMasterData]` warn **ゼロ** = 未充足 / **#251 Scope 1** (トリガー = sinon/proxyquire 導入タスク or Vertex false negative): 該当タスクなし + 6/16 ストームは正しく検知・回復され false negative なし = 未充足 / **#238** (トリガー = ADR-0015 silent failure metric ERROR or 削除済み書類の検索ヒット報告): search_index ERROR 30 日ゼロ + 報告なし = 未充足 (Issue 自身が「実質 P3」明記) → **3 件とも着手せず終了が最適解と提案、user 合意**。**環境ノート**: `.envrc` の `CLOUDSDK_ACTIVE_CONFIG_NAME=doc-split` が `switch-client.sh` の named config activate を上書きするため、環境別 gcloud read は **コマンド単位で `CLOUDSDK_ACTIVE_CONFIG_NAME=<config>` を明示 prefix** して実行 ([feedback_direnv_env_var_in_bash_subshell.md](memory/feedback_direnv_env_var_in_bash_subshell.md) 既知パターン)。**Issue Net**: 起票 0 / close 0 = **Net 0** (ADR ワークフロー完結 + P2 トリガー未充足の実証が質的進捗)。残 open Issues は **#503 / #251 / #238 (P2、トリガー未充足を 2026-07-02 実証済み、明示指示待ち) + #492 (postponed)** の 4 件。次セッション候補 = (a) P2 3 件は各 Issue 本文のトリガー充足時のみ着手 (本セッションで未充足確認済みのため再検証は充足シグナル発生時のみで可)、(b) 健全性レポート定常観察 (429 resilience は実戦実証済みで自動回復見込み)。

**更新日 (前)**: 2026-06-12 session90 (**Vertex AI 429 RESOURCE_EXHAUSTED resilience 強化完遂 — kanameone 健全性レポート 21 件 error 確定事象 (2026-06-11 39 min quota 枯渇) を契機に 429 系専用 retry policy + error 自動 rescue 構造改修 (PR #516 + ADR-0017) → dev/kanameone/cocoro 3 環境 functions deploy 完了 → kanameone 11 件 (自然減 21→11) reprocess 完走確認、Net 0**)。session89 catchup で「即着手 0 件 / セッション終了推奨」だったところに健全性レポート画像 (kanameone 21 件 error、すべて [VertexAI.ClientError] 429 Too Many Requests / RESOURCE_EXHAUSTED) を受領 → user 要件「dev 環境で bugfix + 予防策をあてる。各本番クライアントへ反映。kanameone についてエラー復旧できる。再発しないと言い切れる状況にテスト含めて安心できる」を分解。**「絶対再発しない」は外部 API 依存ゆえ約束不可** ([feedback_promise_overengineering.md](memory/feedback_promise_overengineering.md)) と user に正直に伝え、代替ゴール **「error 状態に留まる事案ゼロ (自動回復保証)」** に置換 → 承認を得て `/impl-plan` → Codex MCP セカンドオピニオン (thread `019eb9bd-...`、主防御は retry window 拡張、rescue は補助 backstop に格下げ、jitter ±20% 必須、初回 delay 3min→1min 短縮を採用) → Phase A 実装 → 評価ゲート全通過 → Phase B-E 展開。**主防御**: `MAX_RETRY_COUNT_429=8` + `RETRY_DELAYS_429_MS=[1,3,6,12,24,48,60,60]` 分 + jitter ±20% で cumulative horizon 約 3.5h (39 分 quota 枯渇に余裕)、非 429 transient は既存挙動完全維持 (MAX_RETRY_COUNT=5 / retryAfter 1min)。**補助 (backstop)**: `rescueErroredDocuments` で 1h+ 経過の 429 系 error doc を pending 復帰、`meta/ocrRescueState.lastErrorRescueAt` で 1h interval guard (既存 1min cadence processOCR に統合、別 Cloud Scheduler 追加なし)、`errorRescueCount >= MAX_ERROR_RESCUE_COUNT (3)` で永続ループ防止。**派生フィールド追加プロトコル (#178 教訓)**: `getReprocessClearFields()` に `errorRescueCount`/`lastRescuedAt` 追加 + `fix-stuck-documents.js` に新フィールド + retryAfter の `deleteField()` 追加 (handleProcessingError の error 確定にも `retryAfter: deleteField()` 追加で一貫性向上)。**Quality gate**: Pure tests 1453 PASS + Integration tests 17 PASS (`functions/test/rescueErroredIntegration.test.ts` 新規、Firestore emulator) + TypeScript build / ESLint errors 0 + `/safe-refactor` (QUOTA_ERROR_MESSAGE_KEYWORDS 単一定数化で is429Error/isQuotaErrorMessage の drift 不可、テストタイトル明示化) + `/code-review medium` (7 angles → 派生フィールドプロトコル違反 4 件検出 → 全修正) + Evaluator agent (前提知識なし第三者、AC1-AC6 PASS、Phase B 進行可)。**PR #516** (10 files, +995/-25) main merge `2a27844` (user 番号認可下 squash)。**Phase B-C: 3 環境 deploy**: dev (updateTime 03:36:59) → kanameone (03:46:47) → cocoro (03:47:34) 全 success、新 revision 確認。**Phase D: kanameone 11 件復旧**: 健全性レポート時点 21 件 → 3 時間経過で自然減 11 件 (dry-run run `27393202811` で確定、全件 status=error / lastErrorMessage に 429 系キーワード / 2026-06-11 11:01-11:43 時間帯)、user 番号認可下 `fix-stuck-documents --include-errors` 実行 (run `27393343718`) → 11 件全件 "rescue state cleared" メッセージで pending 変更成功 → processOCR scheduler 03:55-04:10 で 25+ docs OCR completed (Vertex AI 429 既に解消、新 policy の jitter / exponential は不発動で良好) → 確認 dry-run (run `27393974589`) で **「対象のドキュメントはありません」= status=processing/error 0 件確認** = kanameone 完全復旧確定。**マルチクライアント運用マトリクス完了**: bugfix 実装 ✅ / dev テスト全 PASS ✅ / functions deploy 3 環境 ✅ / functions logs 確認 ✅ / kanameone 11 件 reprocess 完走 ✅ / 全環境 error=0 ✅。**Issue Net**: 起票 0 / close 0 = **Net 0** (構造改修は予防インフラ追加で実害修復としては既存 21 件 → 11 件 → 0 件、明示指示の構造的進捗)。残 open Issues は **#503 / #251 / #238 (P2 enhancement、明示指示待ち) + #492 (postponed)** の 4 件のみ (session89 から変化なし)。次セッション候補 = (a) 残 P2 Issues は明示指示なき限り着手不可、(b) 健全性レポート長期観察で rescueErroredDocuments の実発火確認 (本番 quota 枯渇再発時、ただし自動回復見込み)、(c) ADR-0017 Status を Accepted に格上げ判定 (運用 1-2 週間後)。
