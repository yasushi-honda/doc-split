...
> **注意（Issue #984 段階2a / ADR-0026）**: 日付形・数字/`_` のみの 1〜2 文字トークンは索引に登録しない。`force-reindex.js` の `tokensToRemove` はこの除外を考慮していないため、除外トークンの索引文書が存在しない書類では削除側の NOT_FOUND を書類単位で失敗として扱う可能性がある。全件 `--all-drift` は日付トークンを持たない期待 hash との差で大量に検出される（tokenHash が全書類で変わるため）。復旧指標として `drift` 件数をそのまま使わないこと。書類が更新されて再索引されると、その書類の `search.tokens` から除外トークンが消えるため、飽和した索引文書（`2026` `20` 等）に残る旧 posting は `search.tokens` からは辿れない。将来これらを掃除する場合は `search.tokens` ではなく索引文書側の `postings` マップを走査すること。また `force-reindex.js` / `migrate-search-index.js` はビルド済みの `functions/lib` を使うため、実行前に `functions` を再ビルドしないと除外前のトークナイザーで日付トークンを再登録してしまう。
...
**注意 (Issue #687, BulkWriter化以降)**: `--all-drift --execute` は docId 単位で並行処理される
(デフォルト並行数 5、`--concurrency=<n>` で調整可)。**同一環境に対する force-reindex.js の
cross-process 同時実行 (複数ターミナル/複数 GitHub Actions 実行の同時起動) は引き続き未対応**。
同一 docId への外部からの二重書込みは df の不整合を招くため、同一環境への実行は 1 プロセスに
限定すること (GitHub Actions では `concurrency` 設定での多重起動防止を検討)。
...
**並行数の調整 (Issue #687)**: docId 単位の並行処理数はデフォルト 5。頻出トークン (同一顧客名・
同一事業所等) への書込みが集中し ABORTED リトライが増える場合は下げる、逆に drift 件数が非常に
多く時間内完了が優先の場合は上げることを検討:

```bash
FIREBASE_PROJECT_ID=<env> node scripts/force-reindex.js --all-drift --execute --concurrency=8
```

`--batch-size` は Firestore クエリのページング件数、`--concurrency` は docId の並行処理数で
**別軸のパラメータ**。混同しないこと。
...
2. `--all-drift --missing-hash-only --sample=N --execute` で**分割実行**する(GitHub Actions では `--sample` の選択肢が
   ...
   (実測の目安: 2026-09-20 時点で `"2026"` が 14,562 postings に達して書込みが失敗している。1 文書に入る postings は
   概ね 1.4 万件台が上限で、書類の属性によって前後する。正本は実測の件数と、`--dry-run` の出力)。
...
### 4.5 `stage: documents_search_update` / `stage: search_index_postings_write` / `stage: search_index_postings_remove` で失敗した場合 (半端状態)
...
**BulkWriter化後の追加注意 (Issue #687)**: `stage: "search_index_postings_write"` および
`stage: "search_index_postings_remove"` の失敗も、一部 token の書込み/削除だけ成功し他が
失敗した**部分適用の半端状態**でありうる (各 token への書込み/削除は `Promise.allSettled` で
個別に成否判定されるため)。旧実装の batch commit (全 token 一括成功/失敗) とは異なる点に注意。
`search_index_postings_remove` の部分失敗は、旧 posting の削除フィルタが「まだ postings に
残っている docId のみ」を対象に再構築されるため、再実行しても冪等に収束する。

復旧手順 (全 stage 共通):

1. 該当 docId に対して再度 `--doc-id <id> --execute` を実行
2. reindexDocument は再実行安全 (postings は `merge:true` set のため、部分適用済みの token に
   再実行しても不整合は起きない) なので search_index 側は冪等に収束する
