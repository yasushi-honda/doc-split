...
- ⏳ 2026-09-21: `processocr_request_timeout`(processOCRが900秒のrequest timeoutで強制終了された504の検知、PaddleOCRの大型文書で2026-09-21にkanameoneで実発生)は**PR時点では未適用**。マージ後、`setup-monitoring.yml`(冪等、既存のmetric/policyは変更しない)を dev → kanameone → cocoro の順に実行して適用し、`gcloud logging metrics describe processocr_request_timeout`と`gcloud alpha monitoring policies list`で各環境の存在を確認すること(log-based metricは作成後のログのみ計数するため、適用前に発生した504は遡及されない)。適用後、本行を✅に更新する
