...
- **13 種メトリクス** (log-based) を各環境で作成 (うち`processocr_error`は`lifecycle: temporary`、review_by到達後に削除/恒久化を再判断)
- **12 種アラートポリシー** (Cloud Monitoring) を作成 (`search_index_token_skipped`はアラートなし、観測専用)
...
| `processocr_request_timeout` | Cloud Runリクエストログ `httpRequest.status=(499 OR 504)`(`processocr`、request timeout=`PROCESS_OCR_TIMEOUT_SECONDS`=900秒での強制終了。Scheduler の attemptDeadline も900秒のため、Scheduler側が先に切れた場合は499になる) | 1h 以内に 1 件以上 | ADR-0025 PaddleOCR(1ページ約13〜19秒、Gemini時代は約4.5秒)で約46ページ以上の大型文書が900秒予算を超えうることが、2026-09-21にkanameoneで実発生(45ページ文書、最終的に自動回復、取込から完了まで約17分)して判明。強制終了はアプリログを残さず`processocr_error`(`Error processing document`)では検知できないため別metricで検知する。フィルタは実ログ検索で一致を確認済み(kanameone直近30日で該当1件のみ、dev/cocoroは0件。metricの計数とアラート発火のend-to-endは最初の発生まで未検証)。恒久運用(`lifecycle`ラベルなし) |

### アラートポリシー共通パラメータ

- `duration`: 0s (閾値超過で即発火)。例外: `processocr_completed`はabsence条件のため`duration: 1200s`(上表参照)
- `autoClose`:
  - 標準 (`searchindex_oom` / `ocr_*_truncated` / `summary_truncated` / `processocr_completed` / `processocr_error` / `processocr_request_timeout`): 86400s (24h 無発火で自動クローズ)
...
- **検出遅延**:
  ...
  - `processocr_request_timeout` (alignment 1h): 約3-5分
...
- ✅ 2026-09-21 (PR #1001): dev / kanameone / cocoro に `processocr_request_timeout` の metric と alert policy を適用(Setup Monitoring ワークフロー、dev は run `35564774405`(metric は作成されたが直後の alert 作成が「metric がまだ見つからない」で失敗、metric 作成後は最大10分の伝播遅延がある)→ 再実行 `35564921066` で成功、kanameone / cocoro は run `35565021017` / `35565026673`)。3 環境とも `gcloud logging metrics describe` で filter `httpRequest.status=(499 OR 504)`、`gcloud alpha monitoring policies list` で policy が有効・通知チャネル 1 つを実機確認済み。metric は作成後のログのみ計数するため、適用前の 2026-09-21T02:43Z の kanameone 504 は遡及されない。metric の計数とアラート発火の end-to-end は最初の発生まで未検証。metric 作成直後の alert 作成失敗は再実行(冪等)で解消する
...
### claimが既にdivergent化している場合の人手解除(decision-maker確認済み、恒久運用)

kanameoneは`driveFolderClaimRead`が既に有効なため、統合対象のgroupに対応する`(parentId,name)`の
claimが、統合実行より前に(通常のexport処理により)`divergent`化していることがある。
`execute-drive-sibling-merge`はこの場合、ファイル移動は行うが**trashは実行せずskipする**
(manifestの`skipped`に理由が記録される)。この状態を解消するには:

1. `classify-drive-claim-divergence`(Issue #871既存ツール)を実行し、対象claimの状態を確認する
2. `execute-drive-claim-resync`(`mode: 'release-claim'`)で該当claimを`invalidated`へ遷移させる
3. `audit-drive-sibling-duplicates`・`execute-drive-sibling-merge`を再実行する(fingerprintが
   一致していれば今度はtrashまで完了する)

**この解除作業は一度きりの移行対応ではなく、クライアントが業務上手動でのフォルダ作成・移動を
続ける限り、今後も定期的に発生する運用コストとして受容する**(decision-maker確認済み、
[ADR-0028](../adr/0028-drive-full-scope.md) Cons参照)。既存app管理フォルダと衝突する形で
人が新たに同名フォルダを作るたびに、対象customer/category配下のexportが停止し、上記手順での
人手解除が必要になる。発生頻度が高い場合は、Issue #871 SOPの「月間発生件数閾値超過時の根本原因
レビュー」を流用して監視する。
...
- **`token skipped: document size limit` のログ**: `docId=` `skipped=` `tokenIds=` を含む1処理1行(初回索引後の `search` メタ書込みや、集計トリガーの `customerKey` 書込みによるトリガー再発火で、新規書類1件につき複数行出る。dev 実機検証(2026-09-20)では1件で3行)。`tokenIds=` が飽和したトークンの ID。
