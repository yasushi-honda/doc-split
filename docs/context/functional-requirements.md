...
- **日付語と通常語の混在**（例 `田中 2026年4月`）: 通常語だけで索引検索（AND）→ 候補（上限 500 件）を読み込んだ後に `fileDate` で絞る。候補の切り詰め前には日付で絞れないため、日付に合う書類が候補から漏れうる（`truncated: true` で通知）。`fileDate` が無い書類は日付指定の検索に該当しない
