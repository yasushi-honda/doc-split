 ...
 *    「して」(「〜に対して」「〜に関して」等の連用形接続)が未収録だったため、
 *    「波多野千秋様に対してさくら通所介護センターが提供した」のような地の文で
 *    「に」までしかトリムされず、残った「対して」が実在組織名「さくら通所介護センター」に
 *    連結した状態で捏造判定されていた(D2run2で実測、`coveredFacts`には正しい組織名が
 *    同時に載っており、モデルの幻覚ではなくスキャナ側の助詞リスト欠落と判明)。
 *    「した」「する」と同系統の動詞活用形として「して」を追加し解消(上記1-4とは異なり
 *    形態素解析を要さない単純な語彙追加のため、既存設計の範囲内で修正)。
 *
