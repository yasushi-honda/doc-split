 ...
 *
 * 5. (修正済み、ADR-0027 PR2b実機ゲート初回本番run、2026-09-22発見)助詞リストに
 *    「に対して」が未収録だったため、「波多野千秋様に対してさくら通所介護センターが
 *    提供した」のような地の文で「に」までしかトリムされず、残った「対して」が実在組織名
 *    「さくら通所介護センター」に連結した状態で捏造判定されていた(D2run2で実測、
 *    `coveredFacts`には正しい組織名が同時に載っており、モデルの幻覚ではなくスキャナ側の
 *    助詞リスト欠落と判明)。当初は「した」「する」と同系統の動詞活用形として単体の
 *    「して」を追加する案を検討したが、codex review指摘: 単体「して」は組織名自体が
 *    「して」を内部に含む場合(例: 「あしてらすクリニック」)に捏造プレフィックス
 *    (「新あしてらすクリニック」)の検出をすり抜ける新たなバイパス経路を生む
 *    (上記2と同型の問題)。「に関して」「によれば」等の既存の複合語助詞と同じ設計方針で
 *    「に対して」を複合語として追加し解消(単体助詞の追加ではなく、既存設計の範囲内での
 *    語彙拡張)。
 *
 * 6. (修正済み、ADR-0027 PR2bステップ8全10doc×3run正式gate run再実行、2026-09-22発見)助詞
 *    リストに指示語「当該」が未収録だったため、「関係者は三好陽子様、当該事業所、および
 *    医療機関(詳細不明)である」(D9run2、実際は事業所名不明と正直に回答している健全な出力)で
 *    「当該事業所」が固有名詞「当該」+suffix「事業所」として誤って捏造判定されていた。
 *    「当該」は「(前述の)その」を意味する指示語であり組織名の一部になり得ないため、単体助詞
 *    としてリストへ追加しても5番のような新規バイパスは生じない(組織名が「当該」を内部に
 *    含む実例は想定されない)。
 *
 * これら4件はいずれもPR0結果28run全件では実際に発生していない(コーパス回帰テストで
