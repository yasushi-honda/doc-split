#!/bin/bash
# Sarashina要約 Cloud Run 基盤準備 (ADR-0027 PR1b)
#
# 既知3クライアント(dev/kanameone/cocoro)専用。新規クライアント展開(setup-tenant.sh)には
# 統合しない(scripts/setup-paddle-ocr-infra.shと同じ方針、詳細はそちらのコメント参照)。
#
# 作成するもの:
#   1. API有効化: run.googleapis.com / artifactregistry.googleapis.com (既に有効なら no-op)
#   2. Artifact Registry repo: sarashina-summary (docker, asia-northeast1) + cleanup policy
...
#   3. 無権限runtime SA: sarashina-summary-runtime@<project-id>.iam.gserviceaccount.com
#      (role付与ゼロ)
#
# 作成しないもの(意図的、PR1cに委譲。scripts/setup-paddle-ocr-infra.shのPR3→PR4bと
# 同じ分割方針):
#   - Cloud Run サービス本体
#   - roles/run.invoker 付与(対象サービスが実在しないと付与できない)
#   - runtime SAへのactAs(roles/iam.serviceAccountUser)付与
#     (デプロイSAが実際にサービスをデプロイする段階で恒久付与する、
#     docs/context/delivery-and-update-guide.md参照)
#
# 使用方法:
#   ./scripts/switch-client.sh <dev|kanameone|cocoro>   # 先に named config を切替(推奨)
#   ./scripts/setup-sarashina-summary-infra.sh <dev|kanameone|cocoro> [--dry-run] [--replace-cleanup-policy]
#
# 冪等。再実行しても既存リソースは作り直さない。
#
# 実行前に、実行者アカウントへ以下2ロールを一時付与すること(恒久付与ではない、
# docs/context/delivery-and-update-guide.md「Sarashina要約基盤準備のbootstrap権限」参照):
#   roles/artifactregistry.admin
#   roles/iam.serviceAccountCreator
#
# 確認:
#   gcloud artifacts repositories describe sarashina-summary --location=asia-northeast1 --project=<project-id>
#   gcloud iam service-accounts describe sarashina-summary-runtime@<project-id>.iam.gserviceaccount.com

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REGION="asia-northeast1"
REPO="sarashina-summary"
RUNTIME_SA_NAME="sarashina-summary-runtime"
...
echo "=== Sarashina要約基盤準備 (ADR-0027 PR1b) ==="
echo "alias: $ALIAS"
...
if [ "$DRY_RUN" != true ]; then
  ...
  # (`gcloud projects test-iam-permissions` は無効なコマンド、scripts/setup-paddle-ocr-infra.sh
  # で実測確認済み)、Cloud Resource Manager API v3 を直接叩く。
  #
  # curlはHTTPエラー(401/403/404/429/5xx)でも有効なJSONエラーボディをexit 0で返すため、
  # -w '%{http_code}' で明示的にHTTPステータスを確認する。確認しないと、認証切れ等の
  # 実エラーが「4権限とも権限不足」という誤った診断に化けてしまう(実際には権限があっても
  # トークン失効等で誤検知しうる、setup-paddle-ocr-infra.shと同じ理由)。
...
for api in run.googleapis.com artifactregistry.googleapis.com; do
  # `2>/dev/null || true`だと認証切れ等の実エラーも「未有効」と誤認しうるため、
  # set -euo pipefailの下でエラーをそのまま伝播させる(「未有効」は空出力+exit 0であり、
  # これはエラー扱いにならない、setup-paddle-ocr-infra.shと同じ設計)。
...
if [ "$REPO_EXISTS" = true ]; then
  ...
  if [ "$DRY_RUN" = true ]; then
    ...
    gcloud artifacts repositories create "$REPO" \
      ...
      --location="$REGION" \
      --description="Sarashina summary Cloud Run service images (ADR-0027)" \
      --project="$PROJECT_ID" \
...
#
# 比較ロジック自体はread-onlyなので、--dry-run でも常に実行する(mutatingな
# set-cleanup-policies 呼出のみを DRY_RUN で個別にガードする、setup-paddle-ocr-infra.sh
# と同じ設計)。
APPLY_POLICY=false
if [ "$REPO_EXISTS" = false ]; then
  ...
  # 余分なポリシーの追加を見逃す。cleanupPolicies全体を正規化(キーソート)した上で
  # 完全一致比較する(setup-paddle-ocr-infra.shと同じ設計)。
...
if [ "$SA_EXISTS" = true ]; then
  ...
  # (既存SAを無条件でskipすると、手動/部分プロビジョニング等で付与済みの
  # Storage/Firestore/Vertex等のroleがPR1cのCloud Run実行にそのまま残る、
  # setup-paddle-ocr-infra.shと同じ設計)。
  #
  # `2>/dev/null || true`だとresourcemanager.projects.getIamPolicy権限が欠けている
  # 場合(cocoro等の狭いスコープの実行者)でもエラーが握り潰され「role無し」と誤認して
  # 偽の"確認済み"表示に化ける。同権限はREQUIRED_PERMISSIONSに追加済みでpreflightが
  # 先に保証するが、二重の安全としてここでもエラーはset -e下でそのまま伝播させる。
  ...
  if [ "$DRY_RUN" = true ]; then
    ...
    gcloud iam service-accounts create "$RUNTIME_SA_NAME" \
      --display-name="Sarashina Summary Runtime (no roles)" \
      --description="ADR-0027: Sarashina要約 Cloud Run 実行用。Firestore/Storage/Vertexへの権限は一切付与しない" \
...
echo "=== 完了 ==="
echo "[NEXT / PR1c] Cloud Runサービス作成後に以下を実行すること(付与先は都度実測確認):"
echo "  gcloud iam service-accounts add-iam-policy-binding $RUNTIME_SA \\"
echo "    --member=\"serviceAccount:<GitHub deploy SAのメールアドレス>\" \\"
echo "    --role=\"roles/iam.serviceAccountUser\" --project=$PROJECT_ID"
echo "  (actAs権限。--service-account=<runtime SA>を使うgcloud run deployに必要。"
echo "   詳細: docs/context/delivery-and-update-guide.md「Sarashina要約Cloud Runデプロイ」節)"
echo ""
echo "[NEXT / PR4] generateSummaryBatch(PR4で新設予定、現時点では未実装)の実行SAへ"
echo "run.invokerを付与すること(このコマンドはPaddleOCRのprocessOCR呼び出しパターンとは"
echo "無関係。PaddleOCRはCloud Functions(processOCR)からCloud Runを呼び出すが、Sarashina"
echo "要約はgenerateSummaryBatchという別のCloud Functionから呼び出す設計のため、"
echo "processOCRのSAを流用しないこと):"
echo "  gcloud functions describe generateSummaryBatch --gen2 --region=$REGION --project=$PROJECT_ID \\"
echo "    --account=$EXPECTED_ACCOUNT --format='value(serviceConfig.serviceAccountEmail)'"
echo "  gcloud run services add-iam-policy-binding sarashina-summary \\"
echo "    --region=$REGION --project=$PROJECT_ID --account=$EXPECTED_ACCOUNT \\"
echo "    --member=\"serviceAccount:<上記で確認したgenerateSummaryBatchの実行SA>\" --role=\"roles/run.invoker\""
