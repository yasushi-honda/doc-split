...
if [ "$USE_SERVICE_ACCOUNT" = true ]; then
    ...
    echo ""
    echo -e "${BLUE}Gmail連携オプション:${NC}"
    echo "  Gmail自動取得を有効化する場合、開発者に以下のいずれかを依頼:"
    echo "  - 方式3（推奨）: setup-tenant.sh に --gmail-iap オプションを追加"
    echo "    → OAuth設定が全自動。アプリ内で「Gmail連携」ボタンを押すだけ"
    echo "  - 方式1: setup-tenant.sh に --with-gmail オプションを追加"
    echo "    → GCPコンソールでOAuth設定を手動で行う必要あり"
    echo ""
