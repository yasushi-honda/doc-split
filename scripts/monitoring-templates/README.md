...
| `alert-search-index-write-degraded.yaml` | `search_index_write_degraded` | 1 件 / 24h 窓 (incident は 7d 可視化) | Issue #984。高頻度トークンが Firestore の 1MiB 上限に達してスキップされた(`token skipped: document size limit`)、またはサイズ超過と判定できなかった索引書込み失敗(`index write failed`、判定関数が外れた場合の備え)を検知。`lifecycle: temporary`、`review_by: 2026-10-31`。**kanameone では段階2(根本対応)まで常時 open が正常**で、新規飽和は `tokenIds=` に新しい tokenId が現れるかで読む |
