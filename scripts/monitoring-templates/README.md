...
| `alert-processocr-request-timeout.yaml` | `processocr_request_timeout` | 1 件 / 1h | processOCRが900秒のrequest timeoutで強制終了(HTTP 504、Cloud Runリクエストログの`httpRequest.status=(499 OR 504)`。Scheduler側が先に切れた場合は499)。PaddleOCR実測(1ページ約13〜19秒)で約46ページ以上の大型文書が予算超過しうる(2026-09-21 kanameoneで実発生・自動回復)。アプリログを残さないため`processocr_error`では検知できない。実ログ検索でフィルタの一致を確認済み(metricの計数とアラート発火のend-to-endは最初の発生まで未検証)。恒久運用 |
